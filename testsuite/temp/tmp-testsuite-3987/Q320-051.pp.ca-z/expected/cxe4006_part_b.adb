...
package body Cxe4006_Part_B is
   ...
   type B_Body_Tagged_Type is new Cxe4006_Common.Root_Tagged_Type with
   null record;
   ...
   begin
      if Verbose then
         Report.Comment
           ("wrap around test number" & Integer'Image (Test_Number));
      end if;

      case Test_Number is
         when 2_001 =>
            Good_Tag := X'Tag = Cxe4006_Common.Root_Tagged_Type'Tag;
         when 2_002 =>
            Good_Tag := X'Tag = Cxe4006_Part_A1.A1_Tagged_Type_1'Tag;
         when 2_003 =>
            Good_Tag := X'Tag = Cxe4006_Part_A1.A1_Tagged_Type_2'Tag;
         when 2_004 =>
            Good_Tag := X'Tag = Cxe4006_Part_A2.A2_Tagged_Type'Tag;
         when 2_005 =>
            Good_Tag := X'Tag = Cxe4006_Part_B.B_Tagged_Type'Tag;
         when 3_001 =>
            Report.Failed ("test 3001 call should not have been made");
            return;  -- just to avoid extra error messages
         when others =>
            Report.Failed
              ("bad test number in wrap around" & Integer'Image (Test_Number));
      end case;

      if not Good_Tag then
         Report.Failed
           ("unexpected tag value in wrap around test" &
            Integer'Image (Test_Number));
      end if;
