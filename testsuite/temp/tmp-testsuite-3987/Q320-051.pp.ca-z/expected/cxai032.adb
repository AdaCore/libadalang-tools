...
procedure Cxai032 is
   ...
   Previous  : City;
   Iteration : Natural;
   Marks     : array (Natural range 1 .. 6) of Boolean :=
     (others => False); -- 6 = number of data
...
begin
   ...
   Insert (Distances, "BOSTON    ", 5_537.51 * Km);
   Insert (Distances, "NEW-YORK  ", 5_843.60 * Km);
   Insert (Distances, "MADISON   ", 6_682.15 * Km);
   ...
   for C in Distances.Iterate loop
      ...
      if Iteration = 1
        and then
        (Key (C) /= First_Key (Distances) or
         Distances (C) /= First_Element (Distances))
      then
      ...
      if Iteration = 6
        and then
        (Key (C) /= Last_Key (Distances) or
         Distances (C) /= Last_Element (Distances))
      then
   ...
   for C in Distances.Iterate loop
         ...
         case Iteration is
            ...
            when 5 =>
               Query_Element
                 (C,
                  Tamperer'
                    Access);  -- Tampering with elements, prohibited during Query_Element
      ...
      exception
         ...
         when Occur : others =>
            Report.Failed
              ("Tampering raised other exception: " &
...
exception
   when Occur : others =>
      Report.Failed
        ("Exception at global level: " &
         Ada.Exceptions.Exception_Name (Occur));
      Report.Comment
        ("Exception message: " & Ada.Exceptions.Exception_Message (Occur));
      Report.Result;
end Cxai032;
