...
procedure Cxg2008 is
   ...
   package body Float_Check is
      ...
      begin
         ...
         begin
            Z := X * Y;
            Check (Z, Expected, "test 1a -- (0+0i) * (big+big*i)", Mult_Mbe);
            Z := Y * X;
            Check (Z, Expected, "test 1b -- (big+big*i) * (0+0i)", Mult_Mbe);
         exception
         ...
         begin
            Z := U * X;
            Check (Z, Expected, "test 2 -- (tiny,tiny) * (0,0)", Mult_Mbe);
         exception
            when Constraint_Error =>
      ...
      begin
         for I in 1 .. Samples loop
            Low_Sample :=
              (1.0 - Inv_Radix) / Real (Samples) * Real (I) + Inv_Radix;
            Sample (1) := Low_Sample;
            for J in 1 .. Samples loop
               High_Sample :=
                 (Radix - 1.0) / Real (Samples) * Real (I) + Radix;
