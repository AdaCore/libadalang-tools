...
begin
   ...
   Report.Test
     ("CXACC01",
      "Check that the use of 'Class'Output " &
   ...
   begin
      ...
      Process_Order_Block : declare
         ...
         Vip_Order         : Vip_Request         := Vip_Request (Order_3);
      ...
      begin
         ...
         if Subscriber_Order.Location /= Summer_Subscription.Location or
         ...
         if Last_Minute_Order.Location /= Late_Request.Location or
         ...
         if Tc_Summer_Tag /= Ada.Tags.External_Tag (Order_2'Tag) then
            Report.Failed ("Failed tag comparison - 2");
         end if;

         if Tc_Mayoral_Tag /= Ada.Tags.External_Tag (Order_3'Tag) then
            Report.Failed ("Failed tag comparison - 3");
         end if;

         if Tc_Late_Tag /= Ada.Tags.External_Tag (Order_4'Tag) then
   ...
   exception
      when others =>
         Report.Failed ("Delete not properly implemented for Stream_IO");
   end Deletion;
