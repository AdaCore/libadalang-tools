...
procedure C3a0015 is
   ...
   begin
      ...
      if User_Defined_Pool and then First_Free >= My_Pool.First_Free then
         Failed
           (Subtest & " - Allocation didn't consume storage in the pool - 1");
      else
         First_Free := My_Pool.First_Free;
      end if;
      ...
      if User_Defined_Pool and then First_Free /= My_Pool.First_Free then
         Failed
           (Subtest & " - Conversion did consume storage in the pool - 1");
      end if;
      if Y.all /= Value then
         Failed
           (Subtest &
      ...
      if User_Defined_Pool and then First_Free >= My_Pool.First_Free then
         Failed
           (Subtest & " - Allocation didn't consume storage in the pool - 2");
      else
         First_Free := My_Pool.First_Free;
      end if;
      ...
      if User_Defined_Pool and then First_Free /= My_Pool.First_Free then
         Failed
           (Subtest & " - Conversion did consume storage in the pool - 2");
      end if;
      if X.all /= Value then
         Failed
           (Subtest &
...
begin
   Test
     ("C3A0015",
      "Check that a dervied access type has the same " &
   ...
   begin
      ...
      Check2
        ("Standard_Pool/Derived_Derived_Standard_Pool",
         User_Defined_Pool => False);
      Check3
        ("Derived_Standard_Pool/Derived_Derived_Standard_Pool",
         User_Defined_Pool => False);
   end Std;
   ...
   begin
      Check1
        ("User_Defined_Pool/Derived_User_Defined_Pool",
         User_Defined_Pool => True);
      Check2
        ("User_Defined_Pool/Derived_Derived_User_Defined_Pool",
         User_Defined_Pool => True);
      Check3
