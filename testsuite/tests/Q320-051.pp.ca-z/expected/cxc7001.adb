...
procedure Cxc7001 is
   ...
   begin
      ...
      begin
         ...
         if not ((C1 >= C2) and (C2 >= C3)) then
            Report.Failed
              ("Callable value doesn't match for " &
               Ada.Task_Identification.Image (Which));
         end if;
      end;

      if (A_Task'Identity /= Which) or (The_Ids (Item) /= Which) then
         Report.Failed
           ("Identity values don't match for " &
            Ada.Task_Identification.Image (Which));
      end if;
   ...
   exception
      when Program_Error =>
         null; -- expected case
      when others =>
         Report.Failed ("wrong exception: Abort_Task(Null_ID)");
   ...
   exception
      when Program_Error =>
         null; -- expected case
      when others =>
         Report.Failed ("wrong exception: Is_Terminated(Null_ID)");
   ...
   exception
      when Program_Error =>
         null; -- expected case
      when others =>
         Report.Failed ("wrong exception: Is_Callable(Null_ID)");
...
------------------ transition several tasks to "abnormal"
   ...
   exception
      when others =>
         Report.Failed ("A.TI.AT( SO ) - didn't have no effect");
   end;
