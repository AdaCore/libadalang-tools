...
begin
   ...
   begin
      ...
      if not (A_Null_Exception_Id = Ada.Exceptions.Null_Id) then
         Report.Failed
           ("The default initial value of an object of type " &
      ...
      exception
         ...
         when others =>
            Report.Failed
              ("Exception raised by procedure Raise_Exception " &
      ...
      begin
         ...
         exception
            when Exc : others =>
               Save_Occurrence (Saved_Occurrence, Exc);
         end;

         -- Save a Null_Occurrence actual parameter.
         begin
            Save_Occurrence
              (Target => Saved_Occurrence,
         ...
         exception
            when others =>
               Report.Failed
                 ("Value saved from Procedure Save_Occurrence " &
      ...
      exception
         when others =>
            Report.Failed
              ("Unexpected exception raised during evaluation " &
      ...
      begin
         ...
         exception
            when others =>
               Report.Failed
                 ("Value saved from Function Save_Occurrence " &
                  "resulted in an exception, i.e., was not a " &
                  "value of Null_Occurrence");
         end;
      exception
