...
begin
   Report.Test
     ("CC30004",
      "Check the runtime properties of a generic " &
      "formal type within an instance are the same as " &
      "those of the actual type; specifically, a " &
      ...
      when E =>
         if (Arr (K) /= 1) then
            case Arr (K) is
               when 11 =>
                  Report.Failed
                    ("Out actual integer " & "parameter changed global value");
               when 101 =>
                  Report.Failed
                    ("In out actual integer " &
   ...
   declare
      ...
      Date_Array : array (1 .. 3) of Date_Access :=
        (others => new Date'(Month => Dec, Day => 12, Year => 2_012));
      ...
      procedure New_P is new P (Item => Date, Access_Item => Date_Access);
   ...
   exception
      when E =>
         if (Date_Array (J).all /= (Aug, 7, 1_990)) then
            Report.Failed
   ...
   declare
      ...
      function New_F is new F (Item => Date, Access_Item => Date_Access);

   begin  -- Access_and_Func
      Date_Pointer := new Date'(Year => 2_014, Day => 19, Month => Mar);
      ...
      when E =>
         if Arr (K) /= 1.25 then
            Report.Failed
              ("Out or in out actual procedure " &
               "parameter value changed despite " &
               "raised exception - enumeration, procedure");
         end if;
      when others =>
   ...
   declare
      ...
      Arr     : array (1 .. 4) of Color := (others => Green); -- Initial value.
      E : exception;
   ...
   exception
      ...
      when others =>
         Report.Failed ("Wrong exception raised - enumeration, procedure");
   ...
   declare
      ...
      function New_F is new F (Item => Color, Cnst => Orange);
