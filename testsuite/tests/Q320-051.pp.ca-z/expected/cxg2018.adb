...
procedure Cxg2018 is
   ...
   package body Generic_Check is
      ...
      begin
         Check (Exp (1.0 + 0.0 * I) + I, E + I, "exp(1+0i)", Minimum_Error_C);
         Check (Exp ((Pi / 2.0) * I) + 1.0, 1.0 + 1.0 * I, "exp(pi/2*i)", 3.6);
         Check (Exp (Pi * I) + I, -1.0 + 1.0 * I, "exp(pi*i)", 5.2);
         Check (Exp (Pi * 2.0 * I) + I, 1.0 + I, "exp(2pi*i)", 8.3);
      exception
      ...
      begin
         ...
         for Ii in 1 .. Max_Samples loop
            X.Re :=
              Real'Machine ((B - A) * Real (Ii) / Real (Max_Samples) + A);
            for J in 1 .. Max_Samples loop
               X.Im :=
                 Real'Machine ((B - A) * Real (J) / Real (Max_Samples) + A);
...
begin
   Report.Test ("CXG2018", "Check the accuracy of the complex EXP function");
