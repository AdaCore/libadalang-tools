...
begin
   ...
   begin
      ...
      if Tc_Chars_Ptr /= Ics.Null_Ptr then
         Report.Failed
           ("TC_chars_ptr was not set to Null_Ptr by " & "Procedure Free");
      end if;
      ...
      begin
         Tc_Chars_Ptr           := Ics.New_Char_Array (Tc_Char_Array_1);
         Tc_Char_Array (0 .. 2) :=
           Ics.Value (Item => Tc_Chars_Ptr, Length => 3);

         if Tc_Char_Array (0 .. 2) /= Tc_Char_Array_1 (0 .. 2) then
            Report.Failed
      ...
      begin
         ...
         begin
            Tc_Char_Array :=
              Ics.Value (Item => Freedom (True, Tc_Chars_Ptr), Length => 4);
            Report.Failed
      ...
      begin
         ...
         declare
            Tc : Ic.Char_Array :=
              Ics.Value
                (Item   => Tc_Chars_Ptr,
                 Length => Ic.Size_T (Report.Ident_Int (0)));
         begin
            Report.Failed
              ("Function Value (with Length parameter) did not " &
               "raise Constraint_Error when Length = 0");
            if Tc'Length <= Tc_Char_Array'Length then
               Tc_Char_Array (1 .. Tc'Length) :=
                 Tc; -- Block optimization of TC.
            end if;
         end;
