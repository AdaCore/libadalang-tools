...
--      Ada.Containers.Multiway_Trees. Each of the subprograms is
...
begin
   ...
   My_Tree_1.Append_Child (Parent => Root_1, New_Item => Value_In_Array (1));

   declare

      My_Constant_Reference : My_Multiway_Trees.Constant_Reference_Type :=
        My_Tree_1.Constant_Reference
          (Position => My_Multiway_Trees.First_Child (Parent => Root_1));

   begin

      Tampering_Check (Container => My_Tree_1, Where => "Constant_Reference");

   end;

   declare

      My_Reference : My_Multiway_Trees.Reference_Type :=
        My_Tree_1.Reference
          (Position => My_Multiway_Trees.First_Child (Parent => Root_1));

   begin

      Tampering_Check (Container => My_Tree_1, Where => "Reference");
   ...
   begin
      ...
      Test
        (Value =>
           My_Multiway_Trees.Constant_Reference
      ...
      Test_And_Mod
        (Value =>
           My_Multiway_Trees.Reference (My_Tree_1, Position => My_Cursor_1)
             .Element.all,
      ...
      Test
        (Value     => My_Tree_1.Constant_Reference (Position => My_Cursor_1),
      ...
      Test
        (Value     => My_Tree_1 (My_Cursor_1), -- Constant_Indexing
   ...
   begin
      ...
      for E of My_Tree_1 loop
         ...
         Tampering_Check (Container => My_Tree_1, Where => "reading of loop");
      ...
      for C in My_Tree_1.Iterate loop
         ...
         Tampering_Check (Container => My_Tree_1, Where => "reading in loop");

         Total_Out := Total_Out + My_Tree_1 (C);

         if not (My_Tree_1 (C) > Previous_Value) then

            Report.Failed ("In loop not in ascending order");
      ...
      for C in My_Multiway_Trees.Iterate_Subtree (Root_1) loop
      ...
      for E of My_Tree_1 loop
         ...
         Tampering_Check (Container => My_Tree_1, Where => "writing of loop");
      ...
      for I in Array_Bounds_Type loop
         ...
         if My_Multiway_Trees.Element (Position => My_Cursor_1) /= 0.0 then
