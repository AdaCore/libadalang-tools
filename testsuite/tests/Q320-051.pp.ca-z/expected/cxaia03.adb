...
begin
   ...
   for I in Fxaia00.Array_Bounds_Type loop
      ...
      declare
         ...
         procedure My_Query (Key : in My_Key_Type; Element : in String) is
         begin
   ...
   declare
      ...
      procedure My_Process (Position : in My_Indefinite_Hashed_Maps.Cursor) is
      begin
         ...
         New_String : String :=
           Value_In_Ptr_Array
             (Ada.Containers.Count_Type
                (My_Indefinite_Hashed_Maps.Key (Position => My_Cursor_1))).all;

         procedure My_Update (Key : in My_Key_Type; Element : in out String) is
         begin
   ...
   begin
      ...
      My_Map_2.Insert (Key => 3, New_Item => Value_In_Ptr_Array (1).all);
   ...
   begin
      ...
      My_Map_2.Insert (Key => 6, New_Item => Value_In_Ptr_Array (7).all);
   ...
   begin
      ...
      My_Map_2.Include (Key => 6, New_Item => Value_In_Ptr_Array (2).all);
   ...
   begin
      ...
      My_Map_2.Replace (Key => 4, New_Item => Value_In_Ptr_Array (1).all);
   ...
   if My_Map_2.Element (Key => 1) /= Value_In_Ptr_Array (2).all then
   ...
   if My_Map_2.Element (Key => 3) /= Value_In_Ptr_Array (1).all then

      Report.Failed ("Inserted value not as expected #3");

   end if;

   if My_Map_2.Element (Key => 4) /= Value_In_Ptr_Array (1).all then
   ...
   if My_Indefinite_Hashed_Maps.Equivalent_Keys
       (Left  => My_Map_1.First,
        Right => My_Indefinite_Hashed_Maps.Next (My_Map_1.First))
   then
       ...
       (Left  => My_Map_1.First,
        Right =>
          My_Indefinite_Hashed_Maps.Key
            (My_Indefinite_Hashed_Maps.Next (My_Map_1.First)))
   then
   ...
   if not My_Indefinite_Hashed_Maps.Equivalent_Keys
       (Left  => My_Map_1.First,
        Right => My_Indefinite_Hashed_Maps.Key (My_Map_1.First))
   then
