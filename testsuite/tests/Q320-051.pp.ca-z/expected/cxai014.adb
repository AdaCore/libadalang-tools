...
begin
   ...
   declare
      ...
      procedure My_Process (Position : in My_Bounded_Hashed_Sets.Cursor) is
      begin
...
-- Only a small adjustment made so as not to change the order
   ...
   for I in Array_Bounds_Type loop
      ...
      My_Set_1.Replace_Element
        (Position => My_Cursor_1,
         New_Item =>
           My_Bounded_Hashed_Sets.Element (Position => My_Cursor_1) + 1.0);

      My_Set_2.Replace_Element
        (Position => My_Cursor_2,
         New_Item =>
           My_Bounded_Hashed_Sets.Element (Position => My_Cursor_2) + 1.0);
...
-- by the test values
   ...
   if My_Bounded_Hashed_Sets.Overlap (Left => My_Set_3, Right => My_Set_2) then
   ...
   My_Set_3 :=
     My_Bounded_Hashed_Sets.Copy
       (Source =>
          My_Bounded_Hashed_Sets.Union (Left => My_Set_1, Right => My_Set_2),
        Capacity => Capacity_Reqd);
