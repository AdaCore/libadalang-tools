...
procedure Cdb4001 is
   ...
   procedure Test is
      ...
      Obj_Refs : array (Index, Subpool_Id) of Obj_Ref with
         Independent_Components;
      Vec_Refs : array (Index, Subpool_Id) of Vec_Ref with
         Independent_Components;
      ...
      begin
         ...
         for Subp in Subpool_Id loop
            for Idx in Index loop
               Assert
                 (Obj_Refs (Idx, Subp) /= null,
                  "incomplete Obj_Refs initialization");
               Assert
                 (Vec_Refs (Idx, Subp) /= null,
               ...
               case (Integer (Subp) + Integer (Idx)) mod 5 is
                  ...
                  when 2 =>
                     for I in Vec_Refs (Idx, Subp)'Range loop
                        Expected (Vec_Refs (Idx, Subp) (I).Obj.Id) := True;
                     end loop;
...
begin
   ...
   begin
      Test;
      Assert
        (Finalized = (Finalized'Range => True),
