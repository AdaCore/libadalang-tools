...
procedure Cd4051c is
   ...
   for Check_Clause use record
      Disc      at                     0 range 0 ..   Boolean'Size - 1;
      Int_Comp  at 1 * Units_Per_Integer range 0 ..   Integer'Size - 1;
      Char_Comp at 2 * Units_Per_Integer range 0 .. Character'Size - 1;
   end record;
...
begin
   Test
     ("CD4051C",
      ...
      "WHOSE PARENT TYPE IS IS A RECORD TYPE " & "WITH DISCRIMINANTS");
   ...
   if Check_Record.Int_Comp'Last_Bit /= Ident_Int (Integer'Size - 1) then
   ...
   if Check_Record.Char_Comp'Position /= Ident_Int (2 * Units_Per_Integer) then
