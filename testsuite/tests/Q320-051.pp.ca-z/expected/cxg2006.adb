   ...
   package body Generic_Check is
      ...
      procedure Special_Cases is
         type Data_Point is record
            Re, Im, Radians, Degrees, Error_Bound : Real;
         end record;
         ...
         -- to minimize errors in precision introduced by the test. For cases
         -- where Pi is used in the argument we must allow an extra 1.0*MRE
         -- to account for roundoff error in the argument. Where the result
         -- involves a square root we allow an extra 0.5*MRE to allow for
         -- roundoff error.
...
--    Re               Im                     Radians  Degrees  Err    Test #
         (0.0, 0.0, 0.0, 0.0, 4.0),  -- 1
            ...
            (100_000.0, 100_000.0, Pi / 4.0, 45.0, 5.0)); -- 16
...
--pwb-math                   Test_Data (I).Error_Bound);
            ...
            exception
               when Constraint_Error =>
                  Report.Failed
                    ("Constraint_Error raised in test" & Integer'Image (I));
               when others =>
                  Report.Failed ("exception in test" & Integer'Image (I));
            end;
         end loop;
