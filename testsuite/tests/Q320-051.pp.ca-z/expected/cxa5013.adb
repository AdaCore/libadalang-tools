...
begin
   ...
   Report.Test
     ("CXA5013",
      "Check that a discrete random number generator " &
   ...
   declare
      ...
      -- requirements. Occasionally, even a random series of numbers will fail.
      -- In such a case, the test will reset the random number generator and
      -- rerun the test conditions. This constant determines how many times the
      -- random number generator will be reset before any individual test run
      -- is failed.

      Tc_Max_Random_Test_Runs : constant := 3;

      -- The following constant will ensure that multiple attempts of the
      -- complete set of tests are performed in the event of a failure of a
      -- set of test runs.
      ...
      Tc_Trials_Per_Test : Integer := 1_500;
      ...
      package Discrete_Pack is new Discrete_Random
        (Result_Subtype => Discrete_Type);
      ...
      is
         ...
         Random_Array : array (Gen_Pack.Result_Subtype) of Boolean :=
           (others => False);
      ...
      begin
         ...
         for I in 1 .. Trials_Per_Test loop
            -- Set array element to True when a particular array index is
         ...
         -- further testing performed.
            ...
            select
               ...
               -- This asynchronous select has a triggering statement which is
               -- a delay statement, set to an implementation defined number
               -- of seconds for any particular test to execute. The point
               -- here is to allow the implementation to decide how long to run
               -- this test in order to generate an appropriate (i.e., correct)
               -- sample from the Random Number Generator.
               ...
               -- then the result is success; otherwise, a comment is output to
               -- indicate that the random number generator was unsuccessful in
               -- this series of test runs.
               ...
               if All_Values_Present then
                  ...
                  Report.Comment
                    ("Not all numbers within the Range were produced in " &
                     Integer'Image
                       (Integer (Impdef.Delay_Per_Random_Test * 1_000.0)) &
                     " milliseconds or in " &
            ...
            then abort
               ...
               -- of this abortable part is begun. This loop continues until
               -- either a) every value has been produced or b) the triggering
               -- statement times out.
               ...
               loop        -- successful, the test run limit has been reached,
                  ...
                  for I in 1 .. Trials_Per_Test loop
                     ...
                     -- Set array element to True when a particular array index
                  ...
                  -- following evaluation is performed to determine whether the
                  -- test run was successful, or whether an additional test run
   ...
   begin
      ...
      -- Make a series of test runs, checking to ensure that discrete random
      -- number generators produce each value in their result subtype within
      -- a finite number of calls. In each case, if the first test is not
      -- successful, another attempt is made, after a time-dependent reset,
      -- up to a total of 3 runs. This allows an implementation multiple
      -- opportunities to pass the test successfully. Note: The odds of
      -- getting all 100 integer values in 1500 trials are
      ...
      while Tc_Test_Run < Tc_Finite_Number_Of_Tests and not Tc_Success loop
         ...
         Enumeration_Random_Test (Tc_Trials_Per_Test, Tc_Success);
         -- Increment the number of trials that will be performed in the next
         -- test by 50%.
         Tc_Trials_Per_Test := Tc_Trials_Per_Test + Tc_Trials_Per_Test / 2;
      end loop;
