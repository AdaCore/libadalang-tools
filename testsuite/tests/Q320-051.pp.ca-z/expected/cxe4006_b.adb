...
--      designates a type declared in a remote call interface package.
   ...
   begin
      ...
      if Callee /= Common_Spec then
         Report.Failed
           ("test 1001 callee was " & Type_Decl_Location'Image (Callee));
      end if;
      ...
      if Callee /= Part_A1_1_Spec then
         Report.Failed
           ("test 1002 callee was " & Type_Decl_Location'Image (Callee));
      end if;
      ...
      if Callee /= Part_A1_2_Spec then
         Report.Failed
           ("test 1003 callee was " & Type_Decl_Location'Image (Callee));
      end if;

      Start_Test (1_004);  -- remote
      ...
      if Callee /= Part_A2_Spec then
         Report.Failed
           ("test 1004 callee was " & Type_Decl_Location'Image (Callee));
      end if;
      ...
      if Callee /= Part_B_Spec then
         Report.Failed
           ("test 1005 callee was " & Type_Decl_Location'Image (Callee));
      end if;
...
--      when passed as a class-wide type.
   ...
   begin
      ...
      Cxe4006_Part_A2.Call_B (Root_Obj, 2_001, Callee);
      if Root_Obj.Common_Record_Field /= 205 then
         Report.Failed
           ("test 2001 expected 205 received" &
            Integer'Image (Root_Obj.Common_Record_Field));
      end if;
      if Callee /= Common_Spec then
         Report.Failed
           ("test 2001 callee was " & Type_Decl_Location'Image (Callee));
      end if;

      Start_Test (2_002);
      A1_1_Obj.Common_Record_Field := 210;
      Cxe4006_Part_A2.Call_B (A1_1_Obj, 2_002, Callee);
      if A1_1_Obj.Common_Record_Field /= 216 then
         Report.Failed
           ("test 2002 expected 216 received" &
            Integer'Image (A1_1_Obj.Common_Record_Field));
      end if;
      if Callee /= Part_A1_1_Spec then
         Report.Failed
           ("test 2002 callee was " & Type_Decl_Location'Image (Callee));
      end if;

      Start_Test (2_003);  -- remote
      A1_2_Obj.Common_Record_Field := 220;
      Cxe4006_Part_A2.Call_B (A1_2_Obj, 2_003, Callee);
      if A1_2_Obj.Common_Record_Field /= 227 then
         Report.Failed
           ("test 2003 expected 227 received" &
            Integer'Image (A1_2_Obj.Common_Record_Field));
      end if;
      if Callee /= Part_A1_2_Spec then
         Report.Failed
           ("test 2003 callee was " & Type_Decl_Location'Image (Callee));
      end if;

      Start_Test (2_004);  -- remote
      A2_Obj.Common_Record_Field := 230;
      A2_Obj.A2_Component        := "24680135790987654321";
      Cxe4006_Part_A2.Call_B (A2_Obj, 2_004, Callee);
      if A2_Obj.Common_Record_Field /= 238 then
         Report.Failed
           ("test 2004 expected 238 received" &
            Integer'Image (A2_Obj.Common_Record_Field));
      end if;
      if Callee /= Part_A2_Spec then
         Report.Failed
           ("test 2004 callee was " & Type_Decl_Location'Image (Callee));
      end if;

      Start_Test (2_005);
      B_Obj.Common_Record_Field := 240;
      Cxe4006_Part_A2.Call_B (B_Obj, 2_005, Callee);
      if B_Obj.Common_Record_Field /= 249 then
         Report.Failed
           ("test 2005 expected 249 received" &
            Integer'Image (B_Obj.Common_Record_Field));
      end if;
      if Callee /= Part_B_Spec then
         Report.Failed
           ("test 2005 callee was " & Type_Decl_Location'Image (Callee));
      end if;
...
--      remote call interface package.
   ...
   begin
      ...
      Cxe4006_Part_A2.Call_B (Normal_Obj, 3_001, Callee);
   ...
   exception
      ...
      when others =>
         Report.Failed
           ("Program_Error expected but some other exception" &
...
begin  -- CXE4006_B
   ...
   if Cxe4006_Part_A1'Partition_Id = Cxe4006_B'Partition_Id then
      Report.Failed
        ("Partitioning Error - CXE4006_Part_A1 and CXE4006_B" &
         " are in the same partition.");
   end if;
   if Cxe4006_Part_A2'Partition_Id = Cxe4006_B'Partition_Id then
      Report.Failed
        ("Partitioning Error - CXE4006_Part_A2 and CXE4006_B" &
         " are in the same partition.");
   end if;
   if Cxe4006_Part_B'Partition_Id /= Cxe4006_B'Partition_Id then
      Report.Failed
        ("Partitioning Error - CXE4006_Part_B and CXE4006_B" &
