...
package body Ca11011_0 is          -- Package body OS.
   ...
   begin
      Status := False;
      if (File_Table (Key).Descriptor = Default_Descriptor)
   ...
   begin
      if
        ((File_Table (Key).Descriptor = First_File)
         and then (File_Table (Key).Name = An_Ada_File_Name)
         and then (File_Table (Key).Acct_Access = Init_Permission)
         and then not
         ((File_Table (Key).Mode = Default_Mode)
          or else (File_Table (Key).Current_Status = Default_Status)))
      then
         return (True);
      else
         return (False);
