...
begin
   ...
   for I in Fxaia00.Array_Bounds_Type loop
      ...
      begin
         ...
         if not My_Indefinite_Ordered_Sets.Equivalent_Elements
             (Left =>
                My_Indefinite_Ordered_Sets.Element (Position => My_Cursor_1),
              Right => Value_In_Ptr_Array (I).all)
         then
   ...
   declare
      ...
      procedure My_Process (Position : in My_Indefinite_Ordered_Sets.Cursor) is
      begin
      ...
      procedure My_Reverse_Process
        (Position : in My_Indefinite_Ordered_Sets.Cursor)
      is
      begin
   ...
   if not
     (My_Indefinite_Ordered_Sets.Element
   ...
   My_Set_3 :=
     My_Indefinite_Ordered_Sets.Union (Left => My_Set_1, Right => My_Set_2);
   ...
   if not My_Keys.Equivalent_Keys
       (Left  => My_Keys.Key (Position => My_Cursor_3),
        Right => My_Key_Type (Character'Pos (Value_In_Ptr_Array (9).all (1))))
   then
