...
procedure Cdb0001 is
   ...
   Specified_Size   : constant := 1_000;
   Max_Storage_Size : constant :=
     Specified_Size + Impdef.Maximum_Adjustment_To_Specified_Storage_Size;
...
begin
   ...
   begin
      ...
      Report.Failed ("No exception raised when reserved space " & "exceeded");
      ...
      when Storage_Error =>
         for I in 1 .. Place_I_Stopped loop
            if Acc_Array (I).all /= Ident_Int (I) then
               Report.Failed
                 ("Incorrect value for Acc_Array (" & Integer'Image (I) & ")");
            end if;
         end loop;
      when others =>
         Report.Failed
           ("Wrong exception raised when reserved space " & "exceeded");
   end;
