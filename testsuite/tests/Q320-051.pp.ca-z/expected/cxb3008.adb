...
begin
   ...
   Report.Test
     ("CXB3008",
      "Check that functions imported from the " &
...
--      type Acc_ptr is access IC.char_array;
      ...
      Source_Ptr, Target_Ptr : Ics.Chars_Ptr;
   ...
   begin
      ...
      if Char_Target (0 .. 21) /= Char_Source (0 .. 21) then
         Report.Failed
           ("Incorrect result from the imported version of " & "strcpy - 1");
      end if;

      if String_Length (Char_Target) /= 21 then
         Report.Failed
           ("Incorrect result from the imported version of " & "strlen - 1");
      end if;
      ...
      if Char_Target (0) /= Char_Source (0) then
         Report.Failed
           ("Incorrect result from the imported version of " & "strcpy - 2");
      end if;
      ...
      if Ics.Value (Target_Ptr) /= Tc_String then
         Report.Failed
           ("Incorrect result from the imported version of " & "strcpy - 3");
      end if;

      if String_Length (Ics.Value (Target_Ptr)) /= Tc_String'Length then
         Report.Failed
           ("Incorrect result from the imported version of " & "strlen - 3");
      end if;
      ...
      if Double_Result + String_To_Double (Char_Source) /= 5_150.00 then
         Report.Failed
