...
begin
   ...
   declare
      ...
      package Cef is new Ada.Numerics.Generic_Complex_Elementary_Functions
        (Complex_Pack);
   ...
   begin
      ...
      if not A_Zero_Result (Arcsin (Complex_Zero)) then
         Report.Failed
           ("Non-zero result from Function Arcsin with zero " & "input");
      end if;
      ...
      if not A_Zero_Result (Arctan (Complex_Zero)) then
         Report.Failed
           ("Non-zero result from Function Arctan with zero " & "input");
      end if;
      ...
      if not A_Zero_Result (Arcsinh (Complex_Zero)) then
         Report.Failed
           ("Non-zero result from Function Arcsinh with zero " & "input");
      end if;
      ...
      if not A_Real_Result (Arccos (Complex_Zero)) then
         Report.Failed
           ("Non-real result from Function Arccos with zero input");
      end if;

      if not A_Real_Result (Arccot (Complex_Zero)) then
         Report.Failed
           ("Non-real result from Function Arccot with zero input");
      end if;
      ...
      if not An_Imaginary_Result (Arccoth (Complex_Zero)) then
         Report.Failed
           ("Non-imaginary result from Function Arccoth with " & "zero input");
      end if;
      ...
      if Sqrt (Plus_One) /= Plus_One then
         Report.Failed
           ("Incorrect result from Function Sqrt with input " &
            "value of one");
      end if;
      ...
      if not A_Zero_Result (Log (Plus_One)) then
         Report.Failed
           ("Non-zero result from Function Log with input " & "value of one");
      end if;

      if not A_Zero_Result (Arccos (Plus_One)) then
         Report.Failed
           ("Non-zero result from Function Arccos with input " &
            "value of one");
      end if;

      if not A_Zero_Result (Arccosh (Plus_One)) then
         Report.Failed
           ("Non-zero result from Function Arccosh with input " &
            "value of one");
      end if;
      ...
      if not A_Real_Result (Arcsin (Plus_One)) then
         Report.Failed
           ("Non-real result from Function Arcsin with input " &
            "value of one");
      end if;
      ...
      if not An_Imaginary_Result (Log (Minus_One)) then
         Report.Failed
           ("Non-imaginary result from Function Log with a " &
      ...
      if not A_Real_Result (Arcsin (Minus_One)) then
         Report.Failed
           ("Non-real result from Function Arcsin with a " &
            "minus one input value");
      end if;

      if not A_Real_Result (Arccos (Minus_One)) then
         Report.Failed
           ("Non-real result from Function Arccos with a " &
            "minus one input value");
      end if;
      ...
      if not An_Imaginary_Result (Log (Plus_I)) then
         Report.Failed
           ("Non-imaginary result from Function Log with an " &
            "input value of ""+i""");
      end if;
      ...
      exception
         when others =>
            Report.Failed
              ("Exception raised during the exponentiation of " &
