...
procedure Cxai006 is
   ...
   -- increasing order as this is what determines the order for a set.
...
begin
   ...
   if not My_Ordered_Sets.Equivalent_Sets (Left => My_Set_1, Right => My_Set_2)
   then
   ...
   declare
      ...
      procedure My_Process (Position : in My_Ordered_Sets.Cursor) is
      begin
      ...
      procedure My_Reverse_Process (Position : in My_Ordered_Sets.Cursor) is
      begin
   ...
   if not
     (My_Ordered_Sets.Element (My_Ordered_Sets.Previous (My_Cursor_1)) <
      My_Cursor_1)
   then
   ...
   if My_Ordered_Sets.Overlap (Left => My_Set_3, Right => My_Set_2) then

      Report.Failed ("Erroneously thinks has overlap");

   end if;

   if not My_Ordered_Sets.Is_Subset (Subset => My_Set_2, Of_Set => My_Set_1)
   then
   ...
   if My_Ordered_Sets.Is_Subset (Subset => My_Set_3, Of_Set => My_Set_1) then
   ...
   if not My_Ordered_Sets.Overlap (Left => My_Set_1, Right => My_Set_2) then

      Report.Failed ("Erroneously doesn't think has overlap");

   end if;

   My_Set_3 := My_Ordered_Sets.Union (Left => My_Set_1, Right => My_Set_2);
   ...
   My_Set_3 :=
     My_Ordered_Sets.Intersection (Left => My_Set_1, Right => My_Set_2);
   ...
   My_Set_3 :=
     My_Ordered_Sets.Difference (Left => My_Set_1, Right => My_Set_2);
   ...
   My_Set_3 :=
     My_Ordered_Sets.Symmetric_Difference
   ...
   if My_Ordered_Sets.Element (Position => My_Cursor_3) < Value_In_Array (9) or
   ...
   if not My_Keys.Equivalent_Keys
       (Left  => My_Keys.Key (Position => My_Cursor_3),
        Right => My_Key_Type (Value_In_Array (9)))
   then
