...
with Ada.Containers.Indefinite_Multiway_Trees, Report, Ada.Exceptions;
               ...
               when '"' | ''' =>
                  ...
                  begin
                     ...
                     if Within_Start_Tag then
                        Report.Comment
                          ("Unexpected string within tag " & Text.all);
                        raise Xml_Syntax_Error;
                     end if;
               ...
               when 'a' .. 'z' | 'A' .. 'Z' =>
                  ...
                  begin
                     if not Within_Start_Tag then
                        Report.Comment
                          ("Unexpected unquoted string " & Attr_Name.all);
                        raise Xml_Syntax_Error;
                     end if;
...
--Report.Comment("Reading nested elements");
   ...
   Input : constant String :=
     "<ul align='center'><li>Item1</li><li>Item2</li></ul>";
...
begin
   ...
   begin
      Report.Comment
        ("Selecting item First_Child(Last_Child(First_Child(Root)))");
      Report.Comment
        ("Selected Item is of Kind " & Node_Kinds'Image (Selected_Item.Kind));
      if Selected_Item.Kind /= Text_Kind then
         Report.Failed ("Expected Text_Kind");
      elsif Selected_Item.Text.all /= "Item2" then
         Report.Failed
           ("Expected ""Item2"", found """ & Selected_Item.Text.all & '"');
      elsif Child_Count (Selected_Item_Cursor) /= 0 then
         Report.Failed
           ("Expected no children, found" &
            Ada.Containers.Count_Type'Image
              (Child_Count (Selected_Item_Cursor)));
      elsif Child_Count (Parent (Parent (Selected_Item_Cursor))) /= 3 then
         Report.Failed
           ("Expected 3 children of grandparent, found" &
            Ada.Containers.Count_Type'Image
              (Child_Count (Parent (Parent (Selected_Item_Cursor)))));
      else
         Report.Comment
           ("Selected Item has text """ & Selected_Item.Text.all & '"');
      end if;
   end;
...
exception
   ...
   when Err : others =>
      Report.Failed
        ("Unexpected exception " & Ada.Exceptions.Exception_Name (Err));
      Report.Result;
