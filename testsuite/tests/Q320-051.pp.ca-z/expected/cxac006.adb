...
procedure Cxac006 is
   ...
   begin
      begin
         Strm_Io.Reset (File => File, Mode => Strm_Io.In_File);
      exception
         when Info : others =>
            Report.Failed
              ('(' &
      ...
      exception
         when Strm_Io.Use_Error =>
            null; -- Probably not positionable.
      end;
   exception
...
begin
   Report.Test
     ("CXAC006",
      "Check that stream files opened in Append_Mode handle " &
   ...
   begin
      ...
      exception
         when Strm_Io.Name_Error =>
            Report.Not_Applicable
              ("Unable to create append stream file - " & "Name_Error");
            Report.Comment ("File_Name is " & File_Name);
            raise Abort_Test;
         when Strm_Io.Use_Error =>
            Report.Not_Applicable
              ("Unable to create append stream file - " & "Use_Error");
            Report.Comment ("File_Name is " & File_Name);
            raise Abort_Test;
         when Info : others =>
            Report.Not_Applicable
              ("Unable to create append stream file - " &
               Ada.Exceptions.Exception_Name (Info));
            Report.Comment
              ("Additional information: " &
               Ada.Exceptions.Exception_Information (Info));
      ...
      begin
         if Strm_Io.Index (File) /= Current_Position then
            Report.Failed
              ("(1) Wrong position for Append_File mode - position =" &
               Strm_Io.Count'Image (Strm_Io.Index (File)));
         end if;
         Report.Comment
           ("(1) Stream file in Append_File mode is positionable");
         Append_Supports_Positioning := True;
      exception
         when Strm_Io.Use_Error =>
            Report.Comment
              ("(1) Stream file in Append_File mode is NOT" & " positionable");
      ...
      begin
         ...
         if not Append_Supports_Positioning then
            Report.Comment
              ("(2) Weird: Append_File mode supports Set_Index, " &
               "but not Index");
         else
            Report.Comment
              ("(2) Expected: Both Set_Index and Index supported");
         end if;
      exception
         when Strm_Io.Use_Error =>
            if Append_Supports_Positioning then
               Report.Failed
                 ("(2) Append_File mode supports Index, but not " &
                  "Set_Index");
            else
               Report.Comment
                 ("(2) Expected: Both Set_Index and Index " & "supported");
      ...
      if Append_Supports_Positioning then
         if Strm_Io.Index (File) /= Current_Position then
            Report.Failed
              ("(2A) Set_Index accepted, but position unchanged - position =" &
               Strm_Io.Count'Image (Strm_Io.Index (File)));
      ...
      if Append_Supports_Positioning and Current_Position = 5 then
         ...
         if Strm_Io.Size (File) /= File_Length or else File_Length /= 5 then
            Report.Failed ("(2B) File Size incorrect after writes");
         end if;
         Write_At_Index (File, Current_Position, 'S', Positioning_Ok => True);
         if Strm_Io.Size (File) /= File_Length then
            Report.Failed
              ("(2C) Set_Index accepted, but file written at end anyway");
      ...
      exception
         when Info : others =>
            Report.Failed
              ("(F) Unable to reopen append stream file - " &
               Ada.Exceptions.Exception_Name (Info));
            Report.Comment
              ("Additional information: " &
               Ada.Exceptions.Exception_Information (Info));
      ...
      exception
         when Strm_Io.Use_Error =>
            Report.Comment
              ("(6) In_File on reopened file does not support positioning");
            In_Supports_Positioning := False;
         when Info : others =>
            Report.Failed
      ...
      begin
         Strm_Io.Set_Mode (File => Another_File, Mode => Strm_Io.Append_File);
      exception
         when Info : others =>
            Report.Comment
              ("(H) Unable to Set_Mode from In_File to " &
      ...
      begin
         ...
         begin
            if Strm_Io.Index (Another_File) /= File_Length + 1 then
               Report.Failed
                 ("(8) Wrong position for Set_Mode(Append_File) - " &
                  "position =" &
         ...
         exception
            when Strm_Io.Use_Error =>
               if Append_Supports_Positioning then
                  Report.Failed
                    ("(8) Use_Error checking position of " &
                     "Set_Mode(Append_File)");
               else
                  Report.Comment
                    ("(8) Unable to check position of " &
      ...
      exception
         when Info : others =>
            Report.Comment
              ("(8) Unable to Set_Mode from Append_File to " &
               "Append_File - exception - " &
               Ada.Exceptions.Exception_Name (Info));
            Report.Comment
              ("  Additional information: " &
   ...
   exception
      ...
      when Info : others =>
         Report.Failed
           ("Unexpected exception - " & Ada.Exceptions.Exception_Name (Info));
         Report.Comment
           ("  Additional information: " &
            Ada.Exceptions.Exception_Information (Info));
         if Strm_Io.Is_Open (File) then
