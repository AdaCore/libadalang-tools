...
procedure Cxg2004 is
   ...
   package body Generic_Check is
      ...
      procedure Sin_Check (A, B : Real; Arg_Range : String) is
         -- test a selection of
      ...
      exception
         when Constraint_Error =>
            Report.Failed ("Constraint_Error raised in sin check");
         when others =>
            Report.Failed ("exception in sin check");
      end Sin_Check;

      procedure Cos_Check (A, B : Real; Arg_Range : String) is
         -- test a selection of
         -- arguments selected from the range A to B.
         --
         -- This test uses the identity
      ...
      exception
         when Constraint_Error =>
            Report.Failed ("Constraint_Error raised in cos check");
         when others =>
      ...
      procedure Special_Angle_Checks is
         type Data_Point is record
            Degrees, Radians, Sine, Cosine     : Real;
            Sin_Result_Error, Cos_Result_Error : Boolean;
         end record;
...
--  degrees radians sine cosine sin_er cos_er test #
         ...
         Y                                                        : Real;
         Sin_Arg_Err, Cos_Arg_Err, Sin_Result_Err, Cos_Result_Err : Real;
      begin
         for I in Test_Data'Range loop
            -- compute error components
            Sin_Arg_Err :=
              abs Test_Data (I).Cosine * abs Test_Data (I).Radians / 2.0;
            Cos_Arg_Err :=
              abs Test_Data (I).Sine * abs Test_Data (I).Radians / 2.0;
