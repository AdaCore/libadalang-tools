...
begin
   ...
   Report.Test
     ("CXF1001",
      "Check that values of 2 and 10 are allowable " &
   ...
   exception
      when others =>
         Report.Failed ("Exception raised in Attr_Check_Block");
   ...
   --   3) Validate that the result is the expected result, regardless of the
      ...
      type Radix_2_Type is delta 0.000_1 digits 10;
      type Radix_10_Type is delta 0.000_1 digits 10;
      ...
      type Result_Array_Type is
        array (1 .. Number_Of_Values) of Result_Record_Type;

      Result_Array : Result_Array_Type :=
        ((50.00, 50.00), (613.00, 613.00), (72.70, 72.70));

      function Repetitive_Radix_2_Add
        (Value : in Radix_2_Type) return Radix_2_Type
      is
      ...
      function Repetitive_Radix_10_Add
        (Value : in Radix_10_Type) return Radix_10_Type
      is
   ...
   begin
      ...
      if Repetitive_Radix_2_Add (0.613) /= Result_Array (2).Rad_2 or
      ...
      if Repetitive_Radix_2_Add (0.072_7) /= Result_Array (3).Rad_2 or
        Repetitive_Radix_2_Add (0.072_7) /=
      ...
      if Repetitive_Radix_10_Add (0.05) /= Result_Array (1).Rad_10 or
        Repetitive_Radix_10_Add (0.05) /=
          Radix_10_Type (Result_Array (1).Rad_2)
      then
         Report.Failed ("Incorrect Radix 10 Result, Case 1");
      end if;

      if Repetitive_Radix_10_Add (0.613) /= Result_Array (2).Rad_10 or
      ...
      if Repetitive_Radix_10_Add (0.072_7) /= Result_Array (3).Rad_10 or
        Repetitive_Radix_10_Add (0.072_7) /=
   ...
   exception
      when others =>
         Report.Failed ("Exception raised in Radix_Block");
   end Radix_Block;
   ...
   exception
      when others =>
         Report.Failed ("Exception raised in Size_Block");
   end Size_Block;
