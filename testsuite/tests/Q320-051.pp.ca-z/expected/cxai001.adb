...
begin
   ...
   for I in reverse Array_Bounds_Type loop
      ...
      if My_Vectors.Element (Position => My_Cursor_2) /= Value_In_Array (I)
      then
   ...
   declare
      ...
      procedure My_Process (Position : in My_Vectors.Cursor) is
      begin
      ...
      procedure My_Reverse_Process (Position : in My_Vectors.Cursor) is
      begin
   ...
   -- in the vector. (In the GNAT v7.1.2 implementation it is still pointing
   -- to the 11th element, but the original Value_In_Array (9) is now the 12th
   ...
   My_Vector_1 :=
     My_Vectors.To_Vector (New_Item => My_Default_Value, Length => Num_Tests);
   ...
   My_Vector_1.Insert_Space
     (Before => My_Vector_1.Last_Index, Count => Num_Tests);
