...
package body Cdb4001_Tracked_Subpools is
   ...
   is
      ...
      procedure Set_Result (Mapping : Map) is
         Position : constant Cursor := Floor (Mapping, Storage_Address);
         Info     : Allocator_Info;
      begin
         Result := null;
         if Position /= No_Element then
            ...
            if Storage_Address <
              Info.Storage_Address + Info.Size_In_Storage_Elements then
               Result := Info.Subpool;
            end if;
         end if;
      end Set_Result;
   ...
   function Create_Tracked_Subpool
     (Pool : in out Tracked_Pool) return not null Tracked_Subpool_Handle
   is
      use Subpools;
   begin
      return
        Result : constant not null Tracked_Subpool_Handle :=
   ...
   is
      ...
      Allocated : constant Allocated_Storage_Ref :=
        new Allocated_Storage
          (Size =>
             Size_In_Storage_Elements + Storage_Count'Max (Alignment - 1, 0));

      Unaligned_Address : constant System.Address := Allocated.Storage'Address;
      ...
      begin
         Insert
           (Mapping, Unaligned_Address,
            Allocator_Info'
              (Unaligned_Address, Allocated.Size, Tracked_Subpool));
      end Update_Pool;
   ...
   begin
      ...
      pragma Assert (Alignment = 0 or else Storage_Address mod Alignment = 0);
   ...
   is
      ...
      procedure Free is new Ada.Unchecked_Deallocation (Tracked_Subpool,
         Tracked_Subpool_Ref);

      Tracked_Subpool : Tracked_Subpool_Ref := Tracked_Subpool_Ref (Subpool);
