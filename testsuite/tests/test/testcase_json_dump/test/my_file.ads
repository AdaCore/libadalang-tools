package My_File is
   ...
   type Shape (K : Shape_Kind := Line; Name_Size : Name_Size_Ty := 30) is record
      ...
      X, Y  : Integer range -100 .. 100;
      case K is
         when Line =>
            X_2, Y_2 : Integer range -100 .. 100;
         when Circle | Ellipse =>
            Radius   : Positive;
            case K is
               when Ellipse =>
   ...
   type Shape_Array is array (T2 range <>) of Shape;
   ...
   procedure Test_Proc (X : in out R; Y : T2; A : String; M : Matrix; Z : R2; D : Shape; V : Shape_Array);
