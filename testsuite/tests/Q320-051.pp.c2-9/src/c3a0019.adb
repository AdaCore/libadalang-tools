...
begin
   Report.Test ("C3A0019", "Check that Constraint_Error is raised when a " &
   ...
   begin
      declare
         type Rec (D : not null Int_Ptr) is
            record
               Dummy : Integer := 0;
            end record;
   ...
   begin
      declare
         type Rec (D : not null Int_Ptr := Null_Init) is
            record
               Dummy : Integer := 0;
            end record;
   ...
   begin
      declare
         type Rec (D : not null Int_Ptr := Non_Null_Init) is
            record
               Dummy : Integer := 0;
            end record;
         RO_8 : Rec;
