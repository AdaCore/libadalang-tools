...
begin  -- Main test procedure.
   Report.Test
     ("C392C07",
      "Check that a dispatching subprogram call is " &
...
-- dynamically tagged operands referring to statically tagged declared objects
...
-- turn the full bank of switches ON dynamically tagged allocated objects
   for Knob in Lamps'Range loop
      Clamp (Lamps (Knob).all, On => True);
   end loop;
...
-- a few function calls as operands
   ...
   Tctouch.Validate ("CCC", "Using function returning class-wide type");
