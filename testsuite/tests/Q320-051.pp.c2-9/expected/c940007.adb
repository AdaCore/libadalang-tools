...
begin
   ...
   Report.Test
     ("C940007",
      "Check internal calls of protected functions" &
   ...
   declare  -- encapsulate the test
      ...
      -- This is the time between synchronizing pulses to the ramps. In reality
      -- one would expect a time of 5 to 10 seconds. In the interests of
      -- speeding up the test suite a shorter time is used
      ...
      protected body Ramp is
         ...
         begin
            Tc_Passage_Total :=
              Tc_Passage_Total + (Pass_Point * Tc_Multiplier);
         end Tc_Passage;
         ...
         -- Keep count of vehicles currently on meter queue - we can't use the
         -- 'count because we need the outcall trigger
      ...
      -- Simulate the arrival of a vehicle at the Ramp_Receiver of Ramp_31 and
      -- the generation of an accompanying carrier task
   ...
   begin -- declare
      ...
      -- First simulate calls to the protected functions and procedures from
      -- without the protected object
      --
      -- CALL FUNCTIONS
      if not
        (Ramp_31.Local_Overload = Clear_Level and
         Ramp_31.Next_Ramp_In_Overload = Clear_Level and
         Ramp_31.Freeway_Overload = Clear_Level)
      then
         Report.Failed ("Initial Calls to Ramp_31 incorrect");
      end if;
      if not
        (Ramp_32.Local_Overload = Clear_Level and
         Ramp_32.Next_Ramp_In_Overload = Clear_Level and
         Ramp_32.Freeway_Overload = Clear_Level)
      then
         Report.Failed ("Initial Calls to Ramp_32 incorrect");
      end if;

      -- Now Simulate the arrival of a vehicle at each ramp to verify basic
      -- paths through the test
      New_Arrival_31;
      New_Arrival_32;
      ...
      if not
        (Ramp_31.Local_Overload = Minimum_Level and
         Ramp_31.Freeway_Overload = Minimum_Level)
      then
         Report.Failed ("Secondary Calls to Ramp_31 incorrect");
      end if;
      if not
        (Ramp_32.Local_Overload = Minimum_Level and
         Ramp_32.Freeway_Overload = Minimum_Level)
      then
         Report.Failed ("Secondary Calls to Ramp_32 incorrect");
      end if;

      -- Now Simulate the arrival of another vehicle at each ramp again causing
      -- INTERNAL CALLS but following different paths (queuing on the meter
      -- etc.)
      New_Arrival_31;
      New_Arrival_32;
      delay Pulse_Time_Delta * 2;  -- allow them to pass through the complex
      ...
      if not
        (Tc_Expected_Passage_Total = Ramp_31.Tc_Get_Passage_Total and
         Tc_Expected_Passage_Total = Ramp_32.Tc_Get_Passage_Total)
      then
