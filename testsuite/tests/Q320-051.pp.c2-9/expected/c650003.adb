...
procedure C650003 is
   ...
   -- Check simple return in an extended return and "normal" extended returns.
   begin
      return Obj : Lim1 do
         Obj.Val := Val;
         ...
         if Val mod Report.Ident_Int (3) = 0 then
            Report.Failed
              ("Simple return did not return for" & Natural'Image (Val));
         end if;
         Obj.Bool := Report.Ident_Bool (False);
      end return;
      Report.Failed
        ("Extended return did not return for" & Natural'Image (Val));
   end Construct;
   ...
   begin
      ...
      Report.Failed
        ("Normal simple return did not return for" & Natural'Image (Val));
   end Exit_Construct;
   ...
   exception
      when Program_Error =>
         if Val mod Report.Ident_Int (4) /= 0 then
            Report.Failed
              ("Exception raised for wrong case" & Natural'Image (Val));
         end if;
   ...
   package body Gen is
      ...
      -- Check simple return in an extended return and "normal" extended
      -- returns.
      begin
         return Obj : Lim2 do
            Obj.Val := Val;
            ...
            if Disc'Pos (Val) mod Report.Ident_Int (2) = 0 then
               Report.Failed
                 ("Simple return did not return for" & Disc'Image (Val));
            end if;
            Obj.Bool := Report.Ident_Bool (False);
         end return;
         Report.Failed
           ("Extended return did not return for" & Disc'Image (Val));
      end Construct;
   end Gen;
...
begin
   Report.Test
     ("C650003",
      "Check that an extended return statement can be " &
   ...
   begin
      ...
      if O7.Val /= 6 or (O7.Bool) then
         Report.Failed
           ("Wrong value for O7 - exited return statement returned");
      end if;
      ...
      if O9.Val /= 7 or (O9.Bool) then
         Report.Failed
           ("Wrong value for O9 - goto out of return statement " & "returned");
      end if;
      ...
      if Ob.Val /= 6 or (Ob.Bool) then
         Report.Failed
           ("Wrong value for OB - exception propagation out of " &
