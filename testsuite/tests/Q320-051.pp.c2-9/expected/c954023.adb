...
procedure C954023 is
   ...
   Tc_Credit_Messages_Expected : constant Integer := Tc_Trans_Range'Last - 1;
...
begin
   ...
   Report.Test
     ("C954023",
      "Requeue from within a protected object" &
   ...
   declare -- encapsulate the test
      ...
      protected body Distributor is
         ...
         begin
            ...
            -- Pass this transaction on to the appropriate computation task
            -- but temporarily hold low-priority transactions under overload
            -- conditions
            case Transaction.Code is
               when Credit =>
                  if Credit_Overloaded and Transaction.Priority /= High then
                     -- use the appropriate queue in the family
                     requeue Hold.Wait_For_Underload
                       (Transaction.Priority) with abort;
                  else
      ...
      protected body Hold is
         ...
         -- Once these are executed the barrier conditions for the entries are
         -- evaluated
         ...
         for Transaction_Numb in Tc_Trans_Range loop  -- TC: limit the loop
            ...
            begin
               if Transaction_Numb = Tc_Trans_Range'First then
                  ...
                  elsif Current_Priority = Medium then
      ...
      begin
         ...
         -- This task will now wait for the requeued rendezvous to complete
         -- before proceeding

         -- After the required computations have been performed return the
         -- Transaction_Record appropriately (probably to an output line
         -- driver)
         null;            -- stub
      ...
      -- Computation task. After the computation is performed the rendezvous in
      -- the original message task is completed.
         ...
         loop
            select
               accept Input (Transaction : Acc_Transaction_Record) do
                  ...
                  -- Now take special action depending on which Message. Note:
                  -- The count gives the order in which the messages are
                  ...
                  if Message_Count < 4 then
                     -- This is one of the first three messages which must be
                     -- High priority because we will set "Overload" after the
                     -- first, which is known to be High. The lower priority
                     -- should be waiting on the queues
                     ...
                     if Message_Count = 5 then
                        -- The two medium priority transactions have now been
                        -- processed - release the Low priority items
                     ...
                     -- Too many transactions have arrived. Duplicates? the
                     -- Debit transaction?
                     Report.Failed ("Credit Task: Too many transactions");
                  end if;
               end Input;
            or
               terminate;
