...
begin
   ...
   declare
      ...
      begin
         for I in Int'Val (Ident_Int (-6)) .. Int'Val (Ident_Int (6)) loop
            begin
               if Sint'Pred (I) /= Sint'Val (Sint'Pos (I) - 1) then
                  Failed ("WRONG " & Str & "'PRED " & "FOR " & Int'Image (I));
               end if;
            exception
               when others =>
                  Failed
            ...
            begin
               if Sint'Succ (I) /= Sint'Val (Sint'Pos (I) + 1) then
                  Failed ("WRONG " & Str & "'SUCC " & "FOR " & Int'Image (I));
               end if;
            exception
               when others =>
                  Failed
