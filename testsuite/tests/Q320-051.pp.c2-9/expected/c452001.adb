...
procedure C452001 is
   ...
   -- type Craters. This equality test should call user-defined equality for
   -- type TC_Planet (checks that Names are equal), then predefined equality
   -- for Craters (ultimately calls user-defined equality for type Circle,
   -- checking that Radii of craters are equal).
   ...
   Viking_1_Lander : C452001_2.Mission :=
     (Craft =>
      ...
      Launch_Date => 1_975);
   -- Viking_1_Orbiter /= Viking_1_Lander if predefined equality from the
   -- array type Spacecraft is used for equality of matching components in type
   -- Mission. If user-defined equality for type Spacecraft is incorporated,
   -- which it should not be by 4.5.2(24/3), then Viking_1_Orbiter =
   -- Viking_1_Lander.
   ...
   Jupiter_Craft : C452001_2.Inventory (1 .. 2) :=
     ((1 => (C452001_2.Voyager, Operational => Report.Ident_Bool (True))),
      (1 => (C452001_2.Voyager, Operational => Report.Ident_Bool (True))));
   -- Voyagers /= Jupiter_Craft if predefined equality from the array type
   -- Spacecraft is used for equality of matching components in type Inventory.
   -- If user-defined equality for type Spacecraft is incorporated, which it
   ...
   Sun_At_Bicentennial : C452001_3.Star_Data :=
     (Item =>
      ...
      Supernova => False);
   -- Sun_Today /= Sun_at_Bicentennial if predefined equality from the untagged
   -- record type Star is used for equality of matching components in type
   -- Star_Data. If user-defined equality for type Star is incorporated, which
   -- it should be by 4.5.2(24/3), then Sun_Today = Sun_at_Bicentennial. [Poor
   -- Pluto! - RLB]
   ...
   -- record type Star is used for equality of matching components in type
   -- Constellation. If user-defined equality for type Star is incorporated,
   -- which it should be by 4.5.2(24/3), then Little_Dipper = Ursa_Minor.

   Tptf_1 : C452001_4.Tagged_Partial_Tagged_Full;
   Tptf_2 : C452001_4.Tagged_Partial_Tagged_Full;
   -- With differing values for Boolean component, user-defined (primitive)
   -- equality returns True, predefined equality returns False. Since full
   -- type is tagged, primitive equality should be used.
   ...
   -- With differing values for Duration component, user-defined (primitive)
   -- equality returns True, predefined equality returns False. Since full
   -- type is an untagged record type, primitive equality should be used
   -- (4.5.2(15/3)).
   ...
   -- (primitive) equality returns True, predefined equality returns False.
   -- Since full type is an untagged array, predefined equality should be used.
...
begin
   ...
   -------------------------------------------------------------------
   -- Tagged type with untagged composite component. Untagged component itself
   -- has tagged components.
   -------------------------------------------------------------------
   ...
   -------------------------------------------------------------------
   -- Partial type and full type are tagged. (Full type must be tagged if
   -- partial type is tagged)
   -------------------------------------------------------------------
   ...
   -------------------------------------------------------------------
   -- Partial type and full type are both untagged, the full type is a record
   -- type.
   -------------------------------------------------------------------

   -- Note: In the current test, primitive "=" is visible, so we need to use
   -- that. If 4.5.2(9.7/3) is repealed and the visible declarations removed,
   -- then this result doesn't change.
   ...
   -------------------------------------------------------------------
   -- Partial type and full type are both untagged, the full type is an array
   -- type.
   -------------------------------------------------------------------

   -- Note: In the current test, primitive "=" is visible, so we need to use
   -- that. If 4.5.2(9.7/3) is repealed and visible declarations removed, then
   -- this result changes to the commented out version below.
