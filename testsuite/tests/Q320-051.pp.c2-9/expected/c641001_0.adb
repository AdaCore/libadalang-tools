...
package body C641001_0 is
   ...
   procedure Check_Illegal_Slice_Reference
     (Slice_Passed : in out String; Index : Natural)
   is
   begin
   ...
   -- if the view conversion is not performed, one of the following checks will
   -- fail (given data passed as 0..9 and then 2..11)
   begin
      Check_Under_Index :  -- index 0 should raise C_E
      begin
         Tctouch.Assert
           (Tl (Report.Ident_Int (0)).Data_Item = "*****",
      ...
      begin
         Tctouch.Assert
           (Tl (Report.Ident_Int (11)).Data_Item = "*****",
