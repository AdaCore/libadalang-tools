...
package Cxe4004_Common is
   ...
   type Discriminated_Record (Disc : Boolean := True) is record
   ...
   type Root_Tagged_Record is tagged record
      In_Root : Integer := 22;
   end record;

   type Extended_1 is new Root_Tagged_Record with record
      In_Extension : Integer := 445;
   end record;

   type Extended_2 is new Root_Tagged_Record with record
