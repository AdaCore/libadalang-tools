...
procedure C760007 is
   ...
   begin
      ...
      -- in order to allow for the anonymous objects referred to in the
      -- reference manual, the check for calls to Adjust must be in a
      -- range. This number must then be further adjusted to allow for the
      -- optimization that does not call for an adjust of an aggregate initial
      -- value built directly in the object

      if C760007_0.Adjust_Count not in Min .. Max then
         Report.Failed
           (Message & " = " & Natural'Image (C760007_0.Adjust_Count));
      end if;
   ...
   begin
      ...
      if C760007_0.Child_Adjust_Count not in Min .. Max then
         Report.Failed
           (Message & " = " & Natural'Image (C760007_0.Child_Adjust_Count));
      end if;
...
--      aggregate is controlled.
   ...
   begin
      ...
      -- optional: these assignments may call Adjust:
      --   1: on the value resulting from the aggregate
      --   2: on Object once it's been assigned
