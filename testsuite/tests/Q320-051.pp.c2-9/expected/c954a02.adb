...
begin  -- Main program.
   ...
   --
   -- Call the entry Verify_Results. The entry call will not be accepted until
   -- after Print_Request has been requeued to Done_Printing.
   ...
   if Printer (1).Is_Done then
      ...
      -- Simulate an application which needs access to the printer within a
      -- specified time, and which aborts the current printer job if time
      -- runs out.
      ...
      -- Verify (A): that Print_Request has been aborted. Note: the test will
      -- hang if the task as not been aborted
      --
      while not Print_Request'Terminated loop
         delay Impdef.Minimum_Task_Switch;
      end loop;

      -- Verify (B): that the Done_Printing entry call was canceled, and the
      -- corresponding entry body was not executed.
      --
      -- Set the barrier of the entry to true, then check that the entry body
      -- is not executed. If the entry call is NOT canceled, the entry body
      -- will execute when the barrier is set true.
