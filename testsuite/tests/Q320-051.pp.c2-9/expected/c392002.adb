...
procedure C392002 is
   ...
   subtype Engine_Size is Integer range 100 .. 1_000;

   Five_Tons                  : constant Natural           := 10_000;
   Catalytic_Converter_Offset : constant Vehicle_Emissions := 0.8;
   Truck_Adjustment_Factor    : constant Vehicle_Emissions := 1.2;

   Engine_Emission_Factor : Engine_Emissions_By_Decade :=
     (C1970 => 10.00, C1980 => 8.00, C1990 => 5.00);
   ...
   Cycle_1970 : Sim_1970.Motorcycle :=
     (Weight => 400, Wheels => 2, Size_Of_Engine => 100);
   ...
   -- overridden in the parent extended type, and inherited by the
   -- subsequent extended type.

   function Catalytic_Converter_Present
     (V : in Sim_1970.Vehicle'Class) return Boolean
   is
   begin
      return (Sim_1970.Catalytic_Converter (V)); -- Dispatch according to tag.
   end Catalytic_Converter_Present;

   -- Function Air_Quality_Measure performs a dispatching call on a primitive
   -- operation that has been defined for an ancestor type, and overridden in
   -- each subsequent extended type.

   function Air_Quality_Measure
     (V : in Sim_1970.Vehicle'Class) return Vehicle_Emissions
   is
   begin
...
begin  -- Main test procedure.
   ...
   if
     ((Air_Quality_Measure (Cycle_1970) /= 100.00) or
