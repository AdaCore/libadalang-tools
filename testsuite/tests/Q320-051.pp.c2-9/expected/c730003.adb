...
begin
   ...
   begin
      ...
      -- Update value in the components of the card. There are two component
      -- fields named View, although one is not visible for any view of a
      -- Graphic_Card.
      ...
      if Get_Private_View (Top_Card_On_The_Deck) /= Queen_Of_Spades then
      ...
      if Get_Private_View (Top_Card_On_The_Deck) /= Close_To_The_Vest then
         Report.Failed ("Incorrect value in non-visible component - 2");
      end if;

   exception
      when others =>
         Report.Failed ("Exception raised in test block");
