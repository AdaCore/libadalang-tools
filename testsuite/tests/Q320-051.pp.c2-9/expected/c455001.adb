...
begin
   Test
     ("C455001",
      "Check that universal fixed multiplying operators " &
   ...
   if C /= Ident_Fix (1.0) then
      Failed
        ("Incorrect results for multiplication (1) - result is " &
         F3'Image (C));
   end if;
   ...
   if C /= Ident_Fix (1.0) then
      Failed
        ("Incorrect results for division (1) - result is " & F3'Image (C));
   end if;
   ...
   if C /= Ident_Fix (0.625) then
      Failed
        ("Incorrect results for multiplication (2) - result is " &
         F3'Image (C));
   end if;
   ...
   if C /= Ident_Fix (0.625) then
      Failed
        ("Incorrect results for division (2) - result is " & F3'Image (C));
   end if;
   ...
   if C /= Ident_Fix (0.375) then
      Failed
        ("Incorrect results for multiplication (3) - result is " &
         F3'Image (C));
   end if;
   ...
   if C /= Ident_Fix (1.5) then
      Failed
        ("Incorrect results for division (3) - result is " & F3'Image (C));
   end if;

   A := 0.5;
   B := 0.3; -- Function parameter context.
   if Ident_Fix (A * B) not in Ident_Fix (0.125) .. Ident_Fix (0.25) then
      Failed
        ("Incorrect results for multiplication (4) - result is " &
   ...
   if Ident_Fix (A / B) not in Ident_Fix (0.5) .. Ident_Fix (0.75) then
      Failed
        ("Incorrect results for division (4) - result is " & F3'Image (A / B));
      -- Exact = 0.625..., but B is only restricted to the range 0.75 .. 1.0,
      -- so the result can be anywhere in the range 0.5 .. 0.75.
   end if;
   ...
   if R.D /= 3.5 then
      Failed
        ("Incorrect results for multiplication (5) - result is " &
         F1'Image (R.D));
   end if;

   if R.E /= 3.0 then
      Failed
        ("Incorrect results for division (5) - result is " & F2'Image (R.E));
   end if;
   ...
   if C /= Ident_Fix (1.5) then
      Failed
        ("Incorrect results for multiplication (6) - result is " &
         F3'Image (C));
   end if;
   ...
   if C /= Ident_Fix (0.75) then
      Failed
        ("Incorrect results for division (6) - result is " & F3'Image (C));
   end if;
