...
private
   ...
   -- clock. We want to call the improved implementations, so we derive from
   -- Wall_Clock in the private part.
   ...
   --
   -- Outside the scope of the full view, only (A) is visible, so calls from
   -- outside the scope will get the formal parameter names and default from
   -- (A). Both inside and outside the scope, however, the body executed will
   -- be that corresponding to Set_Display of the parent type.
