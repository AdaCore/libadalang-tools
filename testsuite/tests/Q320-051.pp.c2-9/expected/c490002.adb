...
begin
   ...
   -- and Negative_Fixed are rounded to an integral multiple of the small prior
   -- to assignment (as expected), then Positive_Fixed and Negative_Fixed are
   -- already integral multiples of the small, and equal either P_M1 or P_M2
   -- (resp., N_M1 or N_M2). An equality check can determine in which direction
   -- rounding occurred. For example:
   --
   ...
   if (Positive_Fixed = P_M1) then
      Fixed_Subtest (0.114_33, P_M1, "Positive Fixed: literal");
   else
      Fixed_Subtest (0.114_33, P_M2, "Positive Fixed: literal");
   end if;

   if (Negative_Fixed = N_M1) then
      Fixed_Subtest (-467.139_88, N_M1, "Negative Fixed: literal");
   else
      Fixed_Subtest (-467.139_88, N_M2, "Negative Fixed: literal");
   end if;
