...
procedure Cdb3a01 is
   ...
   package Pool_Override_P1 is new P1 (X => Character) with
      Default_Storage_Pool => Pool2;
...
begin
   ...
   Test
     (Name  => "CDB3A01",
      Descr =>
        "Testing the Default_Storage_Pool aspect with generic " &
        "instantiations");
   ...
   exception
      when Err : Fdb3a00.Pool_Overflow =>
         Failed
           (Descr =>
              "Exception raised. Allocating from wrong pool; " &
              "should use pool 1");
         Comment
           (Descr =>
              "Allocated from " & Ada.Exceptions.Exception_Message (Err));
