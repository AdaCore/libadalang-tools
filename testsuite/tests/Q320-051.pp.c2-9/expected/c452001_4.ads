package C452001_4 is
   ...
   procedure Change
     (Object : in out Tagged_Partial_Tagged_Full; Value : in Boolean);

   type Untagged_Partial_Tagged_Full is private;
   procedure Change
     (Object : in out Untagged_Partial_Tagged_Full; Value : in Integer);
   ...
   -- Primitive equality checks that records equate in component S only. This
   ...
   -- Primitive equality checks that components 1..4 are equal only. We make
   -- this visible to be similar to the record case.

private

   type Tagged_Partial_Tagged_Full is tagged record
      B : Boolean   := True;
      C : Character := ' ';
   end record;
   ...
   type Untagged_Partial_Tagged_Full is tagged record
      I : Integer  := 0;
      P : Positive := 1;
   end record;
   -- predefined equality checks that all components are equal
   ...
   type Untagged_Partial_Untagged_Record_Full is record
      D : Duration         := 0.0;
