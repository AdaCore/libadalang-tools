...
procedure C432004 is
   ...
   -- Default values for the components of SampleType_A (Era => Cenozoic, Loc
   -- => null).
   Sample_B : Sampletype_B := (C432004_0.Sampletype_A with Period => Devonian);

   -- Default values from SampleType_A (Era => Cenozoic, Loc => null).
   Sample_D1 : Sampletype_D :=
     (C432004_0.Sampletype_A with Period => Cambrian, Sample_On_Loan => True);

   -- Default values from SampleType_A and SampleType_B (Era => Cenozoic, Loc
   -- => null, Period => Quaternary).
   Sample_D2 : Sampletype_D := (Sampletype_B with Sample_On_Loan => True);

   -- Default values from SampleType_A and SampleType_C (Era => Cenozoic, Loc
   -- => null, Period => Quaternary).
   Sample_E : Sampletype_E := (Sampletype_C with null record);
   ...
   Sample_I2 : Sampletype_I := (Sampletype_G with Sample_On_Loan => False);

   -- Default values from SampleType_H (Era => Mesozoic, Period => Jurassic).
   Sample_J : Sampletype_J := (Sampletype_H with Sample_On_Loan => False);
...
begin
   ...
   Report.Test
     ("C432004",
      "Check that the type of an extension aggregate " &
   ...
   if Sample_B /= (C432004_0.Cenozoic, null, Devonian) then
      Report.Failed
        ("Object of record extension of abstract ancestor, " &
