...
package C432002_0 is
   ...
   type Discriminant (L : Length) is tagged record
      S1 : String (1 .. L);
   end record;
   ...
   type Constrained_Discriminant_Extension is new Discriminant (L => 10) with
   record
      S2 : String (1 .. 20);
   end record;
   ...
   type Once_Removed is new Constrained_Discriminant_Extension with record
      S3 : String (1 .. 3);
   end record;

   type Twice_Removed is new Once_Removed with record
      S4 : String (1 .. 8);
   end record;
   ...
   -- Discriminant of parent extension constrained to the value of an
   -- expression
   type Constrained_Extension_Extension is new New_Discriminant_Extension
     (N => 20) with
   record
      S3 : String (1 .. 5);
   end record;
   ...
   type Multiple_Discriminants (A, B : Length) is tagged record
      S1 : String (1 .. A);
      S2 : String (1 .. B);
   end record;
