...
procedure C761002 is
   ...
   -- check various master-exit scenarios the "Fake" parameters are used to
   -- avoid unwanted optimizations
   ...
   begin
      ...
      exception
         when Oops =>
            Tctouch.Validate ("F", "leaving via exception");
         when others =>
            Report.Failed ("Wrong exception");
      end Catch;
   end Subtest_4;
...
begin  -- Main test procedure.
   ...
   Subtest_2;
   -- Unchecked_Deallocation already finalized one of the two objects
   -- allocated, the other should be the only one finalized at leaving
   -- the scope of the access type.
