...
begin
   ...
   Test_Block : declare
      ...
      User_Exception_1, User_Exception_2 : exception;
      ...
      exception
         when Program_Error =>
            ...
            exception
               ...
               when others =>
                  Report.Failed
                    ("Unexpected exception handled, expecting " &
                     "User_Exception_1");
            end;
            ...
            Inner_Block : begin
               ...
               for I in 1 .. Exc_Number loop
                  begin
                     ...
                     Report.Failed
                       ("Exception not propagated on loop #" &
                        Integer'Image (I));
                  exception
                     when Exc : others =>
      ...
      Reraise_Out_Of_Scope : declare
         ...
         begin
            if Exception_Identity (Exc_Occ) = The_Exception'Identity then
               ...
               Report.Failed
                 ("Incorrect exception handled in " & "Procedure Handle_It");
            end if;
         end Handle_It;
