...
package body C3a2003_0 is
   ...
   procedure Target_Is_Level_0_Nest (Y : access Desig; S : out Result_Kind) is
      ...
      -- parameter, and assigned to an access object whose type A is declared
      -- at some nesting level. Program_Error is raised if the accessibility
      -- level of the operand type is deeper than that of the target type.
   ...
   is
      ...
      Ren : Desig renames X.all;                   -- Renaming of a dereference
   begin                                           -- of an access parameter.
      -- Ren'Access below will always be safe, since the accessibility level
      -- (although not necessarily the static nesting depth) of type of AL will
      -- always be deeper than or the same as that of the actual corresponding
      -- to Y.
      Al := Ren'Access;
