...
begin
   ...
   Report.Test
     ("C940006",
      "Check external calls of protected functions" & " and procedures");
   ...
   declare  -- encapsulate the test
      ...
      protected body Ramp_31 is
         ...
         -- This is notification from the next ramp that it is in overload.
         -- With this provision we only need to sample the next ramp during
         -- adverse conditions.
         procedure Notify is
         begin
         ...
         begin
            if Next_Ramp_Alert then
               -- EXTERNAL FUNCTION CALL FROM FUNCTION Get next ramp's current
               -- state
      ...
      protected body Ramp_32 is
         ...
         begin
            if Local_State = Clear_Level then
               -- Notify "previous" ramp to check this one for current state.
               -- Subsequent changes in state will not send an alert When
               -- the situation clears another routine performs the all_clear
               -- notification. (not shown) EXTERNAL CALL OF PROCEDURE FROM
               -- PROCEDURE
               Ramp_31.Notify;
            end if;
            Local_State := Sensor_Level;
   ...
   begin -- declare
      ...
      -- Test driver. This is ALL test control code Simulate calls to the
      -- protected functions and procedures from without the protected
      -- object, these will, in turn make the external calls.
      ...
      if not
        (Ramp_31.Local_Overload = Minimum_Level and
         ...
         Ramp_31.Freeway_Overload = Serious_Level)
      then
