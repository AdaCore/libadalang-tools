package body C324001_0 is
   ...
   begin
      return Result : constant Color := Red do
         ...
         pragma Assert (Result not in Another_Color);
      end return;
   end Not_Another_One;

   function Is_Good (X : No_Defaults) return Boolean is
   begin
      return X.Acc /= null;
   end Is_Good;
   ...
   begin
      return Result : constant No_Defaults := (Comp => 0, Acc => null) do
   ...
   begin
      return
        Result : constant Defaults := (Comp => 0, Acc => new String'("xxx")) do
         pragma Assert (Is_Good (Result));
   ...
   begin
      return Result : constant Defaults := (Comp => 0, Acc => null) do
         pragma Assert (not Is_Good (Result));
