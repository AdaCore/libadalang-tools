...
package C452001_0 is
   ...
   type Colored_Circle is new Circle with record
      Color : Colors := White;
   end record;

   function "=" (L, R : Colored_Circle) return Boolean;
   -- Override predefined equality for this tagged type. Predefined equality
   -- should incorporate user-defined (primitive) equality from type Circle.
   -- See C340001 for a test of that feature.

   -- Equality is overridden to ensure that predefined equality incorporates
   -- this user-defined function for any composite type with Colored_Circle
   -- as a component type. (i.e., the type extension is recognized as a tagged
   -- type for the purpose of defining predefined equality for the composite
   -- type).
