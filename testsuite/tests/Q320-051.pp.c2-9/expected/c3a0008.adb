...
begin
   ...
   Area := Integrate_Lookup.Integrate (Integrate_Lookup.Log'Access, 1.0, 2.0);

   if not Integrate_Lookup.Tc_Log_Call or Area /= 0.2 then
      Report.Failed ("Incorrect Log result");
   end if;

   Area := Integrate_Lookup.Integrate (Integrate_Lookup.Sine'Access, 1.0, 2.0);

   if not Integrate_Lookup.Tc_Sine_Call or Area /= 0.0 then
      Report.Failed ("Incorrect Sine result");
   end if;

   Area := Integrate_Lookup.Integrate (Integrate_Lookup.Cos'Access, 1.0, 2.0);
