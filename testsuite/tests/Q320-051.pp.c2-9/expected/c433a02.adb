...
begin
   Report.Test
     ("C433A02",
      "Check that the array component expressions in an " &
      "array aggregate may have a limited type. " &
      "Part B: Two-dimensional arrays");

   F433a00.Reset_Init;
   declare
      O_10 : Str := (1 => (1 => <>, 2 => <>), 2 => (1 => <>, 2 => <>));
      O_11 : Str                 := (1 .. 2 => (1 .. 2 => <>));
      O_12 : Str                 := (1 | 2 => (1 | 2 => <>));
   ...
   declare
      O_20 : Tr (1 .. 2, 1 .. 3) :=
        (1 =>
         ...
         2 =>
           (1 => (Info => (A => 2, C => 'D'), P => <>, T => <>),
            2 => (Info => (A => 2, others => <>), P => <>, T => <>),
            3 => (Info => (A => 2, C => <>), P => <>, T => <>)));
      O_21 : Str :=
        (1 .. 2 =>
           (1 .. 2 => (Info => (A => <>, C => 'E'), P => <>, T => <>)));
      O_22 : Str :=
        (1 | 2 => (1 | 2 => (Info => (A => <>, C => 'F'), P => <>, T => <>)));
      O_23 : Tr (1 .. 3, 1 .. 2) :=
        (others => (others => (Info => <>, P => <>, T => <>)));
   begin
      Check (O_20, 2, "O_20");
      Check (O_21, 4, "O_21");
      Check (O_22, 4, "O_22");
