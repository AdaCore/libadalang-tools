...
begin
   ...
   if (User_Task_Pkg.Tc_Times_Obtained /= 0) or
   ...
   -- Task 1 still waiting to accept Release_Printer, still holds resource Task
   -- 2 queued on Semaphore.Request
   ...
   -- request for resource should be denied and task 3 queued on
   -- Semaphore.Request
   ...
   -- Task 1 has released resource and completed Task 2 has seized the resource
   ...
   then
      ...
      if Verbose then
         ...
         Report.Comment
           ("User 1 Has_Access:" &
            Boolean'Image
              (Printer_Mgr_Pkg.Instantiation.Has_Access (User_Rec_1)));
         Report.Comment
           ("User 2 Has_Access:" &
            Boolean'Image
              (Printer_Mgr_Pkg.Instantiation.Has_Access (User_Rec_2)));
      end if;
   end if;
   ...
   while not (Task1'Terminated and Task2'Terminated and Task3'Terminated) loop
      delay Impdef.Minimum_Task_Switch;
   end loop;
