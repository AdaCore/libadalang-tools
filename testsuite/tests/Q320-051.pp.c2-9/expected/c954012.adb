...
procedure C954012 is
   ...
   --
   Base_Time : Ada.Calendar.Time := Ada.Calendar.Time_Of (1_959, 3, 9);
      ...
      for Transaction_Numb in Tc_Trans_Range loop  -- TC: limit the loop
         ...
         begin
            if Transaction_Numb = Tc_Trans_Range'First then
               ...
               -- For this test send the last transaction to the Debit task to
               -- improve the mix
   ...
   begin
      loop
         ...
         or when not Credit_Overloaded.Value =>
            accept Wait_For_Underload (Transaction : Acc_Transaction_Record) do
               requeue Credit_Computation.Input with abort;
            end Wait_For_Underload;
         or
            accept Tc_Credit_Ok;
            -- We need this to insure that we evaluate the guards at least once
            -- when Credit_Overloaded is False. Otherwise, tasks could stay
            -- queued on Wait_for_Underload forever (starvation).
         or
            terminate;
         end select;
      end loop;
   ...
   begin
      loop
         select
            accept Input (Transaction : Acc_Transaction_Record) do
               if Credit_Overloaded.Value and Transaction.Priority = Low then
               ...
               if Message_Count = 3 then
                  -- The two high priority transactions created subsequent to
                  -- the overload have now been processed
...
begin -- c954012
   Report.Test
     ("C954012",
      "Requeue within an accept body" & " to another entry in the same task");

   Line_Driver.Start;  -- Start the test

   -- Ensure that the message tasks complete before reporting the result
   while (Tc_Tasks_Completed.Count < Tc_Credit_Messages_Expected) or
     (not Tc_Debit_Message_Complete.Value)
   loop
      delay Impdef.Minimum_Task_Switch;
   end loop;
