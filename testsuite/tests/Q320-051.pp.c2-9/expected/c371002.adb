...
begin
   ...
   ---------------------------------------------------------
   declare
      type Rec1 (D3 : Integer) is record
         C1 : Rec_W_Disc (D3, Func1);      -- Func1 evaluated, value 1.
      end record;

      Chk1 : Boolean := Chk (Func1_Cons, 1, "Func1 not evaluated for Rec1");
   ...
   declare
      type Rec_Of_Rec_01 (D3 : Integer) is record
         C1 : Rec_W_Disc (D3, Func1);      -- Func1 evaluated, value -10.
      end record;                          -- Constraint_Error not raised.

      type Rec_Of_Myarr_01 (D3 : Integer) is record
         C1 : My_Array (Func1 .. D3);      -- Func1 evaluated, value -9.
      end record;                          -- Constraint_Error not raised.

      type Rec_Of_Rec_02 (D3 : Integer) is record
         C1 : Rec_W_Disc (D3, 1);
      end record;

      type Rec_Of_Myarr_02 (D3 : Integer) is record
         C1 : My_Array (D3 .. 1);
      end record;
   ...
   begin
      ...
      begin
         declare
            type Acc_Rec_Myarr is
              access Rec_Of_Myarr_02;               -- No Constraint_Error
            -- raised for either
