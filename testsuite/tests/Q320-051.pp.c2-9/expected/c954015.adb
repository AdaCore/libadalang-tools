...
procedure C954015 is
      ...
      loop
         select
            accept Input (Transaction : Acc_Transaction_Record) do
               ...
               -- Depending on transaction content send it on to the some other
               -- task for further processing TC: Arbitrarily send the message
               -- on to Credit_Sub_1
      ...
      loop
         select
            accept Input (Transaction : Acc_Transaction_Record) do
               ...
               -- Depending on transaction content send it on to the some other
               -- task for further processing Arbitrarily send the message on
               -- to Credit_Sub_2
   ...
   -- This is the last in the chain of tasks to which transactions will be
   -- requeued
   --
      ...
      loop
         select
            accept Input (Transaction : Acc_Transaction_Record) do
               ...
               -- until the second arrives. If any of the requeues are blocked
               -- the test will hang here indicating failure
