...
package body C392011_2.C392011_3 is
   ...
   begin
      ...
      -- build a list that looks like: Level_2, Level_1, Level_2, Level_1,
      -- Level_0
      --
      ...
      The_List :=
        new Ragged_Element'
          (new C392011_1.Level_1'(C392011_1.Create), The_List);
      -- Level_1 -> Level_0 >> AC

      The_List :=
        new Ragged_Element'
          (new C392011_2.Level_2'(C392011_2.Create), The_List);
   ...
   begin
      ...
      while Next_Item /= null loop  -- here we go!
         -- these calls better dispatch according to the value in the
         -- particular list item; causing the call to create to dispatch
         -- accordingly. why do it twice? To make sure order makes no
         -- difference
         ...
         -- Here's what's s'pos'd to 'appen: Check( Lev_2, Create ) >> ACEF
         -- Check( Create, Lev_2 ) >> ACEF Check( Lev_1, Create ) >> ACD
         -- Check( Create, Lev_1 ) >> ACD Check( Lev_2, Create ) >> ACEF
         -- Check( Create, Lev_2 ) >> ACEF Check( Lev_1, Create ) >> ACD Check(
         -- Create, Lev_1 ) >> ACD Check( Lev_0, Create ) >> AB Check( Create,
         -- Lev_0 ) >> AB
