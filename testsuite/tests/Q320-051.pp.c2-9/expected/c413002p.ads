...
--     27 Sep 2007  RLB  Converted to ACATS format, then created test from
--                       C413001 by changing objects to access objects.
--     15 Nov 2007  RLB  Fixed typo in objective.
--
--!
package C413002p is
   ...
   --  These subprograms (Base_Proc and Base_Func) are not redefined in the
   --  extension objects. They are used to check the object.operation notation
   --  applied to inherited subprograms (with and without formals).
   ...
   --  These subprograms (Prim_Proc and Prim_Func) are used to check that the
   --  right subprogram is called when they are redefined in extensions
