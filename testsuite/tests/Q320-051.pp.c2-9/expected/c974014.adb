...
begin
   ...
   declare  -- encapsulate test code
      ...
      -- Notify and take corrective action in the event that the procedure
      -- Calculate_Gamma_Function does not converge.
      --
      ...
      --  This is a very time consuming calculation. It is possible, that, with
      --  certain parameters, it will not converge. If it runs for more than
      --  Maximum_Allowable_Time it is considered not to be convergent and
      --  should be aborted.
      --
   ...
   -- we are checking for cancellation/non-cancellation of the delay. We use a
   -- number which, if exceeded, means that the delay was not cancelled and has
   -- proceeded to full term.
   --
