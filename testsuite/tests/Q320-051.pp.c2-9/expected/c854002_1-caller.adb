   ...
   begin
      if Should_Fail then
         begin
            ...
            raise Some_Error;
            -- This raise statement is necessary, because the Report package
            -- has a bug -- if Failed is called before Test, then the failure
            -- is ignored, and the test prints "PASSED". Presumably, this raise
            -- statement will cause the program to crash, thus avoiding the
            -- PASSED message.
         exception
            when Program_Error =>
...
begin
   -- At this point, the bodies of Renamed_F and Renamed_G have not yet been
   -- elaborated, so calling them should raise Program_Error:
