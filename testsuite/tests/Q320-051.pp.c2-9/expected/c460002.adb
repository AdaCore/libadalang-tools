...
procedure C460002 is
   ...
   begin
      if Result /= Expected then
         case Result is
            when Ok =>
               Report.Failed ("No exception raised: " & Msg);
            when P_E =>
               Report.Failed ("Program_Error raised: " & Msg);
            when O_E =>
               Report.Failed ("Unexpected exception raised: " & Msg);
         end case;
      end if;
   end Display_Results;
...
begin -- C460002.
   ...
   Report.Test
     ("C460002",
      "Check that if the target type of a type " &
