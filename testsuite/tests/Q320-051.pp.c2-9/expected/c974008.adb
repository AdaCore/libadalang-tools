...
begin  -- Main program.
   ...
   begin
      ...
      -- Ensure task is waiting at the accept This is the time required
      -- to activate another task and allow it to run to its first accept
      -- statement.
      --
      ...
      then abort
         ...
         -- In other similar tests Validate_Card is called here. In this test
         -- we just check to see if the abortable part is called at all. Since
         -- the triggering call is not queued the abortable part should not be
         -- started
         --
