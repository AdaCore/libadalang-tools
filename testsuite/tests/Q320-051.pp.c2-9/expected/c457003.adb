...
procedure C457003 is
   ...
   exception
      when Constraint_Error =>
         if Invalid_Selector then
            Report.Comment
              ("Constraint_Error raised by case expression " &
      ...
      when Program_Error =>
         if Invalid_Selector then
            Report.Comment
              ("Program_Error raised by case expression " &
               "with invalid selector");
...
begin
   ...
   Report.Test ("C457003", "Check the evaluation of a case expression");
   ...
   begin
      ...
      if Parameter_Value'Valid then
         Report.Comment
           ("Uninitialized value is valid, skipping " &
            "case expression Constraint_Error check");
      else
         begin

            Expression_Value :=
              Do_Case (Parameter_Value, Invalid_Selector => True);
            -- Passing an invalid value; this is a bounded error by 13.9.1(9).
            -- We have to be prepared for either Constraint_Error or
            -- Program_Error to be raised here if the bounded error
            -- is detected.
         ...
         exception
            ...
            when Constraint_Error =>
               Report.Comment
                 ("Constraint_Error raised by passing " &
                  "invalid value, unable to test case expression " &
                  "with invalid value");

            when Program_Error =>
               Report.Comment
                 ("Program_Error raised by passing " &
                  "invalid value, unable to test case expression " &
                  "with invalid value");
