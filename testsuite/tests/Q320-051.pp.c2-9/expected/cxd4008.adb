...
procedure Cxd4008 is
   ...
   Priority_High : constant System.Priority := System.Default_Priority + 5;
   -- priority for the calling tasks - higher than main
   Priority_Tasks : constant System.Priority := System.Default_Priority + 3;
   ...
   task body Checker_2 is
      ...
      Checked                                     : Integer := 0;
      Gate                                        : Boolean := False;
      E1_Gate, E2_Gate, E3_Gate, E4_Gate, E5_Gate : Boolean := True;
   begin
      -- check for order 1-5
      accept Start_Section;  -- enables polling loop
      loop  -- poll for all tasks waiting
         select when Gate and E1_Gate =>
            accept E1 do
               if Last_Accepted >= 1 then
                  Report.Failed
                    ("E1 taken after" & Integer'Image (Last_Accepted));
               end if;
               Last_Accepted := 1;
               Checked       := Checked + 1;
               E1_Gate       := False;
            end E1;
         or when Gate and E2_Gate =>
            accept E2 do
               if Last_Accepted >= 2 then
                  Report.Failed
                    ("E2 taken after" & Integer'Image (Last_Accepted));
               end if;
               Last_Accepted := 2;
               Checked       := Checked + 1;
               E2_Gate       := False;
            end E2;
         or when Gate and E3_Gate =>
            accept E3 do
               if Last_Accepted >= 3 then
                  Report.Failed
                    ("E3 taken after" & Integer'Image (Last_Accepted));
               end if;
               Last_Accepted := 3;
               Checked       := Checked + 1;
               E3_Gate       := False;
            end E3;
         or when Gate and E4_Gate =>
            accept E4 do
               if Last_Accepted >= 4 then
                  Report.Failed
                    ("E4 taken after" & Integer'Image (Last_Accepted));
               end if;
               Last_Accepted := 4;
               Checked       := Checked + 1;
               E4_Gate       := False;
            end E4;
         or when Gate and E5_Gate =>
            accept E5 do
               if Last_Accepted >= 5 then
                  Report.Failed
                    ("E5 taken after" & Integer'Image (Last_Accepted));
               end if;
               Last_Accepted := 5;
               Checked       := Checked + 1;
               E5_Gate       := False;
...
begin
   ...
   Report.Test
     ("CXD4008",
      "Check that protected object and task" &
