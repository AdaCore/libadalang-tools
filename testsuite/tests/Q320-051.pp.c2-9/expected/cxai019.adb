...
--      Ada.Containers.Doubly_Linked_Lists. Each of the subprograms is
...
procedure Cxai019 is
   ...
   package My_Sorting is new My_Doubly_Linked_Lists.Generic_Sorting
...
begin
   ...
   My_List_1.Append (New_Item => Value_In_Array (1));

   declare

      My_Constant_Reference : My_Doubly_Linked_Lists.Constant_Reference_Type :=
        My_List_1.Constant_Reference (Position => My_List_1.First);

   begin

      Tampering_Check (Container => My_List_1, Where => "Constant_Reference");

   end;

   declare

      My_Reference : My_Doubly_Linked_Lists.Reference_Type :=
        My_List_1.Reference (Position => My_List_1.First);

   begin

      Tampering_Check (Container => My_List_1, Where => "Reference");
   ...
   begin
      ...
      Test
        (Value     => My_List_1.Constant_Reference (Position => My_Cursor_1),
      ...
      Test
        (Value     => My_List_1 (My_Cursor_1), -- Constant_Indexing
   ...
   begin
      ...
      for C in My_List_1.Iterate loop
         ...
         if not (My_List_1 (C) > Previous_Value) then

            Report.Failed ("Forward in loop not in ascending order");

         end if;

         Previous_Value := My_List_1 (C);
      ...
      My_Cursor_1 := My_List_1.Find (Item => Value_In_Array (Num_Tests / 2));
      ...
      for C in My_List_1.Iterate (My_Cursor_1) loop
         ...
         Upper_Total_Out := Upper_Total_Out + My_List_1 (C);
      ...
      for C in reverse My_List_1.Iterate (My_Cursor_1) loop
         ...
         Lower_Total_Out := Lower_Total_Out + My_List_1 (C);
      ...
      for C in My_List_1.Iterate loop
         ...
         Early_Total_In := Early_Total_In + My_List_1 (C);

         if not (My_List_1 (C) > Previous_Value) then

            Report.Failed
              ("Early exit forward in loop not in ascending order");

         end if;

         Previous_Value := My_List_1 (C);

         exit when My_Doubly_Linked_Lists."=" (C, My_Cursor_1);

      end loop;

      if not
        (Early_Total_In =
         (Value_In_Array (1) + Value_In_Array (2) + Value_In_Array (3)))
      then
      ...
      for E of My_List_1 loop
         ...
         Tampering_Check (Container => My_List_1, Where => "writing of loop");
      ...
      for I in Array_Bounds_Type loop
         ...
         My_Doubly_Linked_Lists.Next (Position => My_Cursor_1);
