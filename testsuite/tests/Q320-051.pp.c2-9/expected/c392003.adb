...
--
--
-- The following hierarchy of tagged types and primitive operations is utilized
-- in this test:
--
...
-- types will be varied over a series of tests. Locations of declaration and
-- derivation for a particular test are marked with an asterisk (*).
--
-- Root type:
--
--    *  Declared in package.
...
procedure C392003 is
   ...
   -- Root tagged type and primitive operations declared in internal package
   -- (Accounts). Extended types (and primitive operations) derived in nested
   -- packages.
   ...
   package Accounts is
      ...
      function Increment_Bank_Reserve
        (Acct : in Bank_Account) return Dollar_Amount;
      function Assign_Representative
        (Acct : in Bank_Account) return Account_Rep;
      ...
      package S_And_L is
         ...
         -- Function Increment_Bank_Reserve inherited from parent
         -- (Bank_Account).

         -- Primitive operations (Overridden).
         function Assign_Representative
           (Acct : in Savings_Account) return Account_Rep;
      ...
      --=================================================================--
         ...
         package Premium is
            ...
            -- Function Increment_Bank_Reserve inherited twice. Function
            -- Assign_Representative inherited from parent
            --   (Savings_Account).
   ...
   package body Accounts is
      ...
      --=================================================================--
         ...
         package body Premium is
            ...
            begin
               return
                 (Acct.Balance = Preferred_Minimum_Balance and
   ...
   -- Procedures to operate on accounts. Each uses a class-wide IN parameter,
   -- as well as a call to a dispatching operation.
   ...
   -- Function Accumulate_Reserve performs a dispatching call on a primitive
   -- operation that has been defined for the root type and inherited by each
   -- derived type.

   function Accumulate_Reserve
     (Acct : in Accounts.Bank_Account'Class) return Dollar_Amount
   is
   begin
   ...
   -- operation that has been defined in the root type, overridden in the first
   -- derived extended type, and inherited by the subsequent extended type.
...
begin  -- Main test procedure.
   ...
   Preferred_Account_Subtest : begin
      ...
      -- for the Preferred_Account object.
