...
package C730004_0.C730004_2 is                   -- public child
   ...
   -- The following three validation functions provide the capability to check
   -- the limited private types defined in the parent and the private child
   -- package from within the client program.

   function Validate_Create (File : in File_Type) return Boolean;

   function Validate_Modification (File : in File_Type) return Boolean;
...
private
   ...
   Child_File : File_Type :=
     File_Type'(Descriptor => Null_File, Mode => Write_Only, Name => String2);
