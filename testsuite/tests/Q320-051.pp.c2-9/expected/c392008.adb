...
procedure C392008 is
   ...
   -- This procedure will perform a dispatching call on the overridden
...
begin
   ...
   Report.Test
     ("C392008",
      "Check that the use of a class-wide formal " &
      "parameter allows for the proper dispatching " &
      "of objects to the appropriate implementation " &
      "of a primitive operation");

   -- Check the dispatch to primitive operations overridden for each extended
   -- type.
   New_Account (B_Acct);
   New_Account (C_Acct);
   New_Account (Ic_Acct);
