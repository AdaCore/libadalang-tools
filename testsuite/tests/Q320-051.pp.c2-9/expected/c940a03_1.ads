...
package C940a03_1 is
   ...
   -- Models a counting semaphore that will allow up to a specific number of
   -- logins Users (tasks) request a login slot by calling the Request_Login
   -- entry and logout by calling the Release_Login procedure

   Max_Logins : constant Integer := 2;

   type Key_Type is range 0 .. 100;
   -- When a user requests a login, an identifying key will be returned
