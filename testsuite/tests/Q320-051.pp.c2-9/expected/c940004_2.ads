...
-- Printer_Mgr_Pkg
   ...
   -- Instantiate the generic to get code to manage a single printer; User
   -- processes contend for the printer, asking for it by a call to Request,
   -- and relinquishing it by a call to Release

   -- This package extends a tagged type to customize it for the printer in
   -- question, then it uses the type to instantiate the generic and declare
   -- a semaphore specific to the particular resource
