...
package C490002_0 is
   ...
   type My_Fix is delta 0.062_5 range -1_000.0 .. 1_000.0;
...
--
   ...
   --  |----|-------------|-----------------|-------------------|-----------| |
   --  | | | | | 0 P_M1 Less_Pos_Than_Half Pos_Exactly_Half More_Pos_Than_Half
   --  P_M2

   Positive_Real  : constant := 0.114_33;          -- Named number.
   Pos_Multiplier : constant := Float'Truncation (Positive_Real / Small);

   -- Pos_Multiplier is the number of integral multiples of small contained in
   -- Positive_Real. P_M1 is thus the largest integral multiple of small less
   -- than or equal to Positive_Real. Note that since Positive_Real is a named
   -- number and not a fixed point object, P_M1 is generated without assuming
   -- that rounding is performed correctly for fixed point subtypes.
   ...
   -- or lies between P_M1 and P_M2 (since truncation was forced in generating
   -- Pos_Multiplier). It is not certain, however, exactly where it lies
   -- between them (halfway, less than halfway, more than halfway). This
   -- fact is irrelevant to the test.

   -- The following entities are used to verify that rounding is performed
   -- according to the value of 'Machine_Rounds. If language rules are obeyed,
   -- the intermediate expressions in the following static initialization
   -- expressions will not be rounded; all calculations will be performed
   -- exactly. The final result, however, will be rounded to an integral
   -- multiple of the small (either P_M1 or P_M2, depending on the value of
   -- My_Fix'Machine_Rounds). Thus, the value of each constant below will
...
--
   ...
   -- The descriptions for the positive cases above apply to the negative cases
   -- below as well. Note that, for N_M2, 'Pred is used rather than 'Succ.
   -- Thus, N_M2 is further from 0.0 (i.e. more negative) than N_M1.

   Negative_Real  : constant := -467.139_88;       -- Named number.
