...
procedure C341a02 is
   ...
   Ic_Acct     : aliased Interest_Checking.Account := (1_000.00, 10.00, 0.030);
   New_B_Acct  : aliased Bank.Account := (Current_Balance => 20.00);
   New_C_Acct  : aliased Checking.Account          := (200.00, 20.00);
   New_Ic_Acct : aliased Interest_Checking.Account := (2_000.00, 20.00, 0.060);

   -- Define and initialize (by direct assignment) objects of a class-wide type
   -- originating from the root type (Bank.Account).
   ...
   New_Accounts : array (1 .. Max_Accts) of Atm_Card :=
     (1 => New_B_Acct'Access, 2 => New_C_Acct'Access, 3 => New_Ic_Acct'Access);

   -- Define an account auditing procedure with a class-wide variable that can
   -- hold a value of any object within the class, and once initialized, can
   -- hold other values of the same specific type.

   procedure Audit (Num : in Integer; Amt : out Bank.Dollar_Amount) is
