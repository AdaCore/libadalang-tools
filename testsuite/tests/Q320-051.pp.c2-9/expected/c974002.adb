...
procedure C974002 is
   ...
   Start_Time : constant Ada.Calendar.Time :=
     Ada.Calendar.Time_Of (1_901, 1, 1);
...
begin  -- Main program.
   ...
   begin
      ...
      -- Expiry is the time to be specified in the delay_until statement of the
      -- asynchronous select. Since it has already passed, the abortable part
      -- should not execute, and the sequence of statements of the triggering
      ...
      if not Triggering_Alternative_Executed then
         Report.Failed
           ("No delay: triggering alternative sequence " &
   ...
   begin
      ...
      -- Expiry is the time to be specified in the delay_until statement of
      -- the asynchronous select. It should not pass before the abortable part
      -- completes, at which time control should return to the caller; the
      -- sequence of statements of the triggering alternative should not be
      -- executed.
      ...
      if Triggering_Alternative_Executed then
         Report.Failed
           ("Long delay: triggering alternative sequence " &
