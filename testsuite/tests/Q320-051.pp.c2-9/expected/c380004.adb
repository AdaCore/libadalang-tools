...
procedure C380004 is
   ...
   type Rec (D1, D2 : Positive) is record
      null;
   end record;
   ...
   begin
      ...
      begin
         if not Is_Ok (T (X), 16, 16, 17) or not Is_Ok (T (Y), 15, 15, 17) then
      ...
      begin
         ...
         declare
            ...
            X : Arr;
         begin
            if Is_Ok (T (X (1)), 11, 11, 17)
              and then Is_Ok (T (X (2)), 10, 10, 17) then
               null;
            elsif Is_Ok (T (X (2)), 11, 11, 17)
              and then Is_Ok (T (X (1)), 10, 10, 17) then
               null;
            else
         ...
         declare
            ...
            X : Nrec;
         begin
            if Is_Ok (T (X.C1), 8, 8, 17) and then Is_Ok (T (X.C2), 9, 9, 17)
            then
               null;
            elsif Is_Ok (T (X.C2), 8, 8, 17)
              and then Is_Ok (T (X.C1), 9, 9, 17) then
               null;
            else
...
begin
         ...
         loop
            select
               accept Is_Ok (D3 : Integer; E_First : Integer; E_Last : Integer;
                  ...
                  begin
                     if Poe.D3 = D3 then
                        -- Can't think of a better way to check the bounds of
                        -- the entry family.
                        begin
                           Cnt := E (E_First)'Count;
                           Cnt := E (E_Last)'Count;
   ...
   declare
      ...
      protected body Poe is
         ...
         begin
            if Poe.D3 = D3 then
               -- Can't think of a better way to check the bounds of the entry
               -- family.
               begin
                  Cnt := E (E_First)'Count;
                  Cnt := E (E_Last)'Count;
