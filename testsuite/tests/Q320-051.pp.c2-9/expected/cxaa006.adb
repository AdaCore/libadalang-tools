...
begin
   ...
   Report.Test
     ("CXAA006",
      "Check that procedure Put will correctly " &
...
-- file in the most efficient manner possible.
   ...
   Operational_Test_Block : declare
      ...
      function Tc_Mode_Selection
        (Selector : Integer) return Ada.Text_Io.File_Mode
      is
      begin
         case Selector is
            when 1 =>
               return Ada.Text_Io.In_File;
            when 2 =>
               return Ada.Text_Io.Out_File;
            when others =>
               return Ada.Text_Io.Append_File;
         end case;
...
-- lines in this file will be limited to that length.
      ...
      -- Test control code.
      if
        (Integer (Ada.Text_Io.Line (A_Bounded_File)) /=
         Report.Ident_Int (1)) or
        (Integer (Ada.Text_Io.Col (A_Bounded_File)) /= Report.Ident_Int (20))
      then
...
-- place (following the first character).
      ...
      if
        ...
        (Integer (Ada.Text_Io.Col (A_Bounded_File)) /= Report.Ident_Int (12))
      then
...
-- line.
      ...
      if (Natural (Ada.Text_Io.Line (A_Bounded_File)) /= (Tc_Line + 1)) or
        (Integer (Ada.Text_Io.Col (A_Bounded_File)) /= Report.Ident_Int (5))
      then
...
-- verify some or all of the data that is contained there.
         ...
         Ada.Text_Io.Get_Line (A_Bounded_File, First_String, Tc_Width1);
         Ada.Text_Io.Get_Line (A_Bounded_File, Second_String, Tc_Width2);
         Ada.Text_Io.Get_Line (A_Bounded_File, Third_String, Tc_Width3);
         Ada.Text_Io.Get_Line (A_Bounded_File, Fourth_String, Tc_Width4);
