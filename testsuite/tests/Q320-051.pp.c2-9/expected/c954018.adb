...
begin
   ...
   Report.Test
     ("C954018",
      "Requeue: abort the called task" & " while Caller is still queued");
   ...
   -- Delay long enough to ensure that the requeue has "arrived" on the entry
   -- queue. Note: TC_State.Set_On_Entry_Queue is called the statement before
   -- the requeue
   --
   delay Impdef.Switch_To_New_Task;

   -- At this point the Receiver is guaranteed to have the requeue on the entry
   -- queue
   --
   abort Receiver;
   ...
   -- Inform the Intermediate task that the process is complete to allow it to
   -- continue to completion itself
   ...
   if not
     (Tc_State.Original_Caller_Complete and Tc_State.Intermediate_Complete)
   then
