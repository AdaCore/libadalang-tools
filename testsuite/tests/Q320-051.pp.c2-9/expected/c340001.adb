...
procedure C340001 is
   ...
   -- DB_Rec1 = DB_Rec2 using user-defined equality DB_Rec1 /= DB_Rec2 using
   -- predefined equality
   ...
   -- Current = Last using user-defined equality Current /= Last using
   -- predefined equality
...
begin
   ...
   -- equality operations from the parent to compare the inherited components
   ---------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------
   -- Check that equality and inequality for the type extension incorporate the
   -- predefined equality operators for the extended component type
   ---------------------------------------------------------------------
   ...
   -- List1 contents are (2, (1, 2, 3, 0, 0, 0, 0, 0, 0, 0)) List2 contents are
   -- (2, (1, 2, 0, 0, 0, 0, 0, 0, 0, 0))

   -- List1 = List2 using user-defined equality List1 /= List2 using predefined
   -- equality
   ...
   ---------------------------------------------------------------------
   -- RList1 and RList2 are made equal but "different" by adding a nonzero
   -- value to RList1 then removing it. Removal updates the list Length only,
   -- not its contents. The two lists will be equal according to the defined
   -- list abstraction, but the records will contain differing component
   -- values.
   ...
   -- RList1 contents are (2, (1, 2, 3, 0, 0, 0, 0, 0, 0, 0), 'A') RList2
   -- contents are (2, (1, 2, 0, 0, 0, 0, 0, 0, 0, 0), 'A') RList3 contents
   -- are (2, (1, 2, 0, 0, 0, 0, 0, 0, 0, 0), 'B')

   -- RList1 = RList2 if List."=" inherited RList2 /= RList3 if List."="
   -- inherited and extended with Character "="

   ---------------------------------------------------------------------
   -- Check that "=" and "/=" are the user-defined operations inherited from
   -- the parent type.
   ---------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------
   -- Check that "=" and "/=" for the type extension are NOT extended with the
   -- predefined equality operators for the extended component. A limited type
   -- extension should inherit the parent equality operation as is.
   ---------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------
   -- Check that user-defined equality was incorporated and extended with
   -- equality of extended components.
   ---------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------
   -- Check that equality and inequality for the type extension succeed despite
   -- the presence of differing variant parts.
   ---------------------------------------------------------------------
