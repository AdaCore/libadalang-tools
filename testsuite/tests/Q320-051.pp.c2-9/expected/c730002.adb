...
begin
   ...
   Report.Test
     ("C730002",
      "Check that the full view of a private " &
   ...
   begin
      ...
      -- C730002_0.Routine_Maintenance (ancestor version). Give no
      -- second parameter so that the default expression must be used.

      Routine_Maintenance (Engine => Nuclear_Drive);

      -- The value of the Specialist component should equal "Moe", which is
      -- the default value from the ancestor's version of Routine_Maintenance,
      -- and not the default value from the parent's version of
      -- Routine_Maintenance.
      ...
      -- components should be those assigned in the parent type's version of
      -- the body of Routine_Maintenance. Note: Only components associated
      -- with the ancestor type are
      ...
      -- Dispatching Case:
      -- Use a dispatching subprogram to ensure that the correct body is used
      -- at runtime.
      ...
      -- that the body of the parent version of the inherited subprogram was
      -- used.
