...
procedure C432001 is
   ...
   N_Object : C432001_0.N :=
     (How_Long_Ago => Report.Ident_Int (375), Era => C432001_0.Paleozoic);

   P_Object : C432001_0.P; -- default value is (150,
   --                   C432001_0.Mesozoic)

   N_N_Object : C432001_1.N_N := (N_Object with Period => C432001_1.Devonian);

   P_N_Object : C432001_1.P_N := (P_Object with Period => C432001_1.Jurassic);
   ...
   P_N_Object_2 : C432001_1.P_N :=
     (C432001_0.P (P_N_N_Object) with C432001_1.Carboniferous);

   N_N_Object_2 : C432001_1.N_N :=
     (C432001_0.N'(42, C432001_0.Precambrian) with C432001_1.Carboniferous);
...
begin
   ...
   if not C432001_0.Check (N_Object, 375, C432001_0.Paleozoic) then
      Report.Failed
        ("Object of " & "nonprivate type " & "failed content check");
   end if;

   if not C432001_0.Check (P_Object) then
      Report.Failed ("Object of " & "private type " & "failed content check");
   end if;
   ...
   -- "grandparent" ancestor type

   -- types tested are derived through nonprivate extensions only (extension
   -- aggregates are not allowed if the path from the ancestor type wanders
