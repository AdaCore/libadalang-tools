...
procedure C761006 is
   ...
   begin
      ...
      -- should call Adjust
      A_Good_Object :=
        (Ada.Finalization.Controlled with Unique => 0, others => False);
   ...
   begin
      ...
      exception
         when others =>
            Report.Failed ("Adjust caused exception at wrong lex");
   ...
   procedure Finalize_Test is
      ...
      begin
         ...
         Item := Target;
         -- finalization of Item should cause PE ARM7.6:21 allows the
         -- implementation to omit the assignment of the value into an
         -- anonymous object, which is the point at which Adjust is normally
         -- called. However, this would result in Program_Error's being raised
         -- before the call to Adjust, with the consequence that Adjust is
         -- never called.
      ...
      procedure Finalize_17_Outer is
         ...
         begin
            Catch (White);
            -- Note: It is unspecified if Catch deallocates the memory of the
            -- allocated object, and if it ceases to exist. As such, it is
            -- possible that it will be finalized when the scope of the
            -- access type is exited. We check for this case below.
         exception
            when Program_Error =>
               if not Sup.Events_Occurring (Sup.Good_Finalize) then
      ...
      exception
         when others =>
            Report.Failed
              ("Unchecked_Deallocation check, unwanted exception in Outer");
   ...
   begin
      ...
      exception
         when Program_Error =>
            Report.Comment
              ("Unchecked_Deallocation check, double finalization occurred");
         when others =>
            Report.Failed
              ("Unchecked_Deallocation check, unwanted exception in caller");
