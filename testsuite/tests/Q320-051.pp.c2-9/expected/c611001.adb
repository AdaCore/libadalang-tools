...
begin
   ...
   if Count (S) /= 3 then
      Report.Failed
        ("Count should be 3, is actually" & Natural'Image (Count (S)));
   end if;

   Report.Comment ("Pop and verify we get a 33");
   X := Pop (S);
   if X /= 33 then
      Report.Failed ("Expected 33 on Pop, found" & My_Int'Image (X));
   end if;

   Report.Comment ("Pop and verify we get a 22");
   X := Pop (S);
   if X /= 22 then
      Report.Failed ("Expected 22 on Pop, found" & My_Int'Image (X));
   end if;

   Report.Comment ("Verify that Count is now one");
   if Count (S) /= 1 then
      Report.Failed
        ("Count should be 1, is actually" & Natural'Image (Count (S)));
   end if;
   ...
   exception
      ...
      when E : others =>
         Report.Failed
           ("Wrong exception raised: " & Ada.Exceptions.Exception_Name (E));
   end;
   Report.Result;
end C611001;
