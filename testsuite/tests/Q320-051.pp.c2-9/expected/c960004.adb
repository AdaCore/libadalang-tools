...
begin
   ...
   declare  -- To get the Report.Result after all has completed
      ...
      -- This is the carrier task. One of these is created for each message
      -- that requires ACK
      --
      ...
      --:::::::::::::::::::::::::::::::::
      -- There would also be another task type "No_ACK_Task" which would be the
      -- carrier task for those messages not requiring an ACK. This task would
      -- call Send_Message.ACK_Not_Required. It is not shown in this test as it
      -- is not used.
      --:::::::::::::::::::::::::::::::::
      ...
      begin
         ...
         -- Now put the message out to the Send_Message task and wait (no more
         -- than Allowable_Ack_Time) for its completion
         --
         select
         ...
         then abort
            -- If the rendezvous is not completed in the above time, this call
            -- is cancelled
         ...
         loop
            select
               accept Ack_Required (Message_To_Send : Message_Type) do
                  -- Here send the message from within the rendezvous waiting
                  -- for full transmission to complete
                  null;  -- stub
