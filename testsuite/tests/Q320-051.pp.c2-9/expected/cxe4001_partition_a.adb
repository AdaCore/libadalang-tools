...
package body Cxe4001_Partition_A is
   ...
   begin
      if Cxe4001_Decl_Pure.Verbose then
         Report.Comment
           ("Check that a predefined exception can be raised" &
            " in one partition and handled in another");
      end if;

      Cxe4001_Partition_B.Raise_Program_Error;
      Report.Failed
        ("Predefined exception was not propagated across partitions");
   exception
      when Program_Error =>
         null;  -- passed
      ...
      when others =>
         Report.Failed
           ("Wrong exception was propagated across partitions (1)");
   ...
   begin
      if Cxe4001_Decl_Pure.Verbose then
         Report.Comment
           ("Check that an exception declared in a shared " &
   ...
   exception
      ...
      when others =>
         Report.Failed
           ("Wrong exception was propagated across partitions (2)");
   ...
   begin
      if Cxe4001_Decl_Pure.Verbose then
         Report.Comment
           ("Check that an exception declared in another" &
            " partition can be handled with an 'others'" &
            " exception handler");
      end if;
      Cxe4001_Partition_B.Raise_Invisible_Exception;
      Report.Failed
        ("Invisible exception was not propagated across partitions");
   exception
      when System.Rpc.Communication_Error =>
         Report.Failed ("Communication_Error occurred");
      when others =>
   ...
   exception
      ...
      when others =>
         Report.Failed
           ("Wrong exception was propagated across partitions (3)");
   ...
   begin
      ...
      Report.Failed
        ("Local re-raised exception was not propagated" &
         " across partitions");
   exception
      when Exception_Local_To_A =>
         null;  -- passed
      ...
      when others =>
         Report.Failed
           ("Wrong exception was propagated across partitions (4)");
