...
package body Pp.Scanner is
   ...
   Token_To_Symbol_Map : constant array (Same_Text_Kind) of Symbol :=
     (Start_Of_Input | End_Of_Input => Name_Empty, Enabled_LB_Token => Name_NL,
      Disabled_LB_Token | Tab_Token => Name_Empty,
      False_End_Of_Line => Name_Empty, True_End_Of_Line => Name_NL,
      ...
      Arrow            => Intern ("=>"), Dot_Dot => Intern (".."),
      Exp_Op           => Intern ("**"), Not_Equal => Intern ("/="),
      Greater_Or_Equal => Intern (">="), Right_Label_Bracket => Intern (">>"),
      Less_Or_Equal    => Intern ("<="), Left_Label_Bracket => Intern ("<<"),
      Box              => Intern ("<>"), Colon_Equal => Intern (":="),

      --  Ada 83 reserved words

      Res_Abort    => Intern ("abort"), Res_Abs => Intern ("abs"),
      Res_Accept   => Intern ("accept"), Res_Access => Intern ("access"),
      Res_And      => Intern ("and"), Res_All => Intern ("all"),
      Res_Array    => Intern ("array"), Res_At => Intern ("at"),
      Res_Begin    => Intern ("begin"), Res_Body => Intern ("body"),
      Res_Case     => Intern ("case"), Res_Constant => Intern ("constant"),
      Res_Declare  => Intern ("declare"), Res_Delay => Intern ("delay"),
      Res_Delta    => Intern ("delta"), Res_Digits => Intern ("digits"),
      Res_Do       => Intern ("do"), Res_Else => Intern ("else"),
      Res_Elsif    => Intern ("elsif"), Res_End => Intern ("end"),
      Res_Entry    => Intern ("entry"), Res_Exception => Intern ("exception"),
      Res_Exit     => Intern ("exit"), Res_For => Intern ("for"),
      Res_Function => Intern ("function"), Res_Generic => Intern ("generic"),
      Res_Goto     => Intern ("goto"), Res_If => Intern ("if"),
      Res_In       => Intern ("in"), Res_Is => Intern ("is"),
      Res_Limited  => Intern ("limited"), Res_Loop => Intern ("loop"),
      Res_Mod      => Intern ("mod"), Res_New => Intern ("new"),
      Res_Not      => Intern ("not"), Res_Null => Intern ("null"),
      Res_Of       => Intern ("of"), Res_Or => Intern ("or"),
      Res_Others   => Intern ("others"), Res_Out => Intern ("out"),
      Res_Package  => Intern ("package"), Res_Pragma => Intern ("pragma"),
      Res_Private => Intern ("private"), Res_Procedure => Intern ("procedure"),
      Res_Raise    => Intern ("raise"), Res_Range => Intern ("range"),
      Res_Record   => Intern ("record"), Res_Rem => Intern ("rem"),
      Res_Renames  => Intern ("renames"), Res_Return => Intern ("return"),
      Res_Reverse  => Intern ("reverse"), Res_Select => Intern ("select"),
      Res_Separate => Intern ("separate"), Res_Subtype => Intern ("subtype"),
      Res_Task     => Intern ("task"), Res_Terminate => Intern ("terminate"),
      Res_Then     => Intern ("then"), Res_Type => Intern ("type"),
      Res_Use      => Intern ("use"), Res_When => Intern ("when"),
      Res_While    => Intern ("while"), Res_With => Intern ("with"),
      Res_Xor      => Intern ("xor"),

      --  Ada 95 reserved words

      Res_Abstract => Intern ("abstract"),
      Res_Aliased => Intern ("aliased"), Res_Protected => Intern ("protected"),
      Res_Until    => Intern ("until"), Res_Requeue => Intern ("requeue"),
      ...
      Res_Some => Intern ("some")); -- Token_To_Symbol_Map
   ...
   procedure Poke_Kind (Tok : in out Opt_Token; New_Kind : Reserved_Word);
   ...
   procedure Poke_Kind (Tok : in out Opt_Token; New_Kind : Reserved_Word) is
   ...
   function Next_Sloc_First (X : Tokn_Cursor) return Positive is
     (if At_Last (X) then New_Sloc_First (X.V.all) else Sloc_First (Next (X)));
   ...
   --  Number of lines in comment
      begin
      return L * (String'("--")'Length + X.Leading_Blanks)
   ...
   procedure Append_Tokn
     (V : in out Tokn_Vec; X : Token; Org : String := "Append Token");
   ...
   is
      ...
      begin
         ...
         pragma Assert
           (if K not in EOL_Token | Comment_Kind then Length (X) = Text_Len);
         pragma Assert
            ...
            if Is_Empty (V) then
               ...
               begin
                  if Kind (Prev) in EOL_Token | Enabled_LB_Token then
                     ...
                     Result.Line := Prev_Sloc.Line + Num_Lines (Prev) - 1;
                     Result.Col  := Prev_Sloc.Col + Last_Line_Len (Prev);
                  else
                     Result.Line := Prev_Sloc.Line;
                     Result.Col  :=
                       Prev_Sloc.Col + Prev_Sloc.Last - Prev_Sloc.First + 1;
                  end if;
   ...
   begin
      ...
      Append
        (V.Fixed,
         Fixed_Part'
           (K, Sloc_Line => Sloc.Line, Sloc_Col => Sloc.Col,
            Sloc_First   => Sloc.First, Origin => Intern (Org)));
      case K is
         when Same_Text_Kind =>
   ...
   is
      Tok : Token (Kind => X);
   begin
      Tok.Sloc := (First => 1, Last => Str (Text (X)).Length, others => <>);
   ...
   begin
      ...
      Tok.Sloc := (First => 1, Last => To_W_Str (Tx)'Length, others => <>);
   ...
   begin
      ...
      if Count > 0 then
         if Existing_OK and then Kind (Last (V'Unrestricted_Access)) = Spaces
         then
            declare
   ...
   begin
      ...
      return
        Result : constant Boolean := Cur.Fi = Last_Index (Cur.V.Fixed) + 1 do
   ...
   begin
      return not (Kind (X) = Spaces or else Kind (Y) = Spaces); -- ????????????
   ...
   function Lookup_Reserved_Word
     (Text : Symbol; Ada_Version : Ada_Version_Type) return Reserved_Word_Or_Id
   is
   ...
   is
      ...
      begin
         while not Is_Line_Terminator (Cur) and then Cur /= W_NUL loop
            Get;
         end loop;
      end Skip_To_EOL;
      ...
      begin
         loop
            Get;
            exit when not
              (Is_Letter (Cur) or else (Cur = '_' and Lang = Ada_Lang)
               or else Is_Digit (Cur) or else Is_Mark (Cur));
         end loop;
      ...
      begin
         ...
         if Kind_Of_Comment = Other_Whole_Line_Comment
           and then not
           (Is_Letter (Cur) or else Is_Digit (Cur) or else Is_Space (Cur)
            or else Is_Line_Terminator (Cur) or else Cur = '-')
         ...
         Is_Fillable_Comment :=
           Kind_Of_Comment = Other_Whole_Line_Comment and then Is_Space (Cur);
         ...
         begin
            ...
            --  filled comment block.
               begin
               if Is_Fillable_Comment and then not Allow_Short_Fillable
                 and then Too_Short
               then
                  Is_Fillable_Comment := False;
               end if;
               ...
               case Kind_Of_Comment is
                  ...
                  when Other_Whole_Line_Comment =>
                     Tok :=
                       (Other_Whole_Line_Comment, Tok.Sloc,
                        Text           => Intern ("invalid token text"),
                        Leading_Blanks => Leading_Blanks, Width => W,
                        Num_Lines      => 1,
                        Last_Line_Len  => Cur_Col - Tok.Sloc.Col);
      ...
      begin
         Tok.Sloc :=
           (Line => Cur_Line, Col => Cur_Col, First => Cur_First, Last => <>);
         ...
         then
            ...
            case Cur is
               ...
               when '0' .. '9' =>
                  Tok :=
                    (Kind => Numeric_Literal, Sloc => Tok.Sloc, others => <>);
               ...
               when ''' =>
                  ...
                  then -- it's a tick
                     ...
                     Tok :=
                       (Kind   => Character_Literal, Sloc => Tok.Sloc,
                        others => <>);
                     Get;
                     pragma Assert (Cur = ''');
                     Get;
               ...
               when '#' =>
                  case Lang is
                     ...
                     when Ada_Lang =>
                        Tok :=
                          (Kind   => Preprocessor_Directive, Sloc => Tok.Sloc,
                           others => <>);
                        Skip_To_EOL;
                  end case;
               when '$' =>
         ...
         pragma Assert
           (if Tok.Kind in Whole_Line_Comment then
      ...
      Start_Token : constant Token :=
        (Start_Of_Input,
         Sloc =>
           (Line => Cur_Line, Col => Cur_Col, First => Cur_First, Last => 0));
   ...
   begin
      ...
      if X.Kind = Fillable_Comment and then YY.Kind = Other_Whole_Line_Comment
      then
         YY :=
           (Fillable_Comment, YY.Sloc, Text => YY.Text,
            Leading_Blanks => YY.Leading_Blanks, Width => YY.Width,
            Num_Lines => YY.Num_Lines, Last_Line_Len => YY.Last_Line_Len);
      elsif X.Kind = Other_Whole_Line_Comment
        and then YY.Kind = Fillable_Comment
      then
         YY :=
           (Other_Whole_Line_Comment, YY.Sloc, Text => YY.Text,
            Leading_Blanks => YY.Leading_Blanks, Width => YY.Width,
            Num_Lines => YY.Num_Lines, Last_Line_Len => YY.Last_Line_Len);
      end if;
   ...
   begin
      ...
      Text_IO.Put
        (Text_IO.Standard_Output,
         ...
         Capitalize (Kind (Tok)'Img) & " at " & Image (Sloc (Tok)));
      if Kind (Tok) in Comment_Kind | Spaces or else Tokn_Length (Tok) > 2 then
         Text_IO.Put
      ...
      if Show_Origin then
         Text_IO.Put
           (Text_IO.Standard_Output,
            ", origin = """ & Str (Tok.V.Fixed (Tok.Fi).Origin).S & """");
      end if;
   ...
   begin
      return
        Result : Tokn_Cursor :=
          (V, Last_Index (V.Fixed) + 1, Last_Index (V.Octets) + 1) do
   ...
   procedure Put_Tokns (Tok : Tokn_Cursor) is
      ...
      F      : constant Tokn_Cursor := First (Tokens'Unrestricted_Access);
      L : constant Tokn_Cursor := Next (Last (Tokens'Unrestricted_Access));
   begin
