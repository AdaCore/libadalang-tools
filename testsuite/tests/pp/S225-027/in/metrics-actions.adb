...
with Utils.Command_Lines.Common; use Utils;
...
package body METRICS.Actions is
   ...
   --    1 not implemented
     ...
     (Contract          => True,
      ...
      Tagged_Coupling_Out  => True, Hierarchy_Coupling_Out => True,
      Tagged_Coupling_In   => True, Hierarchy_Coupling_In => True,
      Control_Coupling_Out => True, Control_Coupling_In => True,
      Unit_Coupling_Out    => True, Unit_Coupling_In => True);
   ...
   function Is_Ancestor_CU (Anc, Child : Metrix_Ref) return Boolean is
     (Anc /= null and then Child /= null
      and then
      (Anc = Child or else Is_Ancestor_CU (Anc, Specs (Child.Child_Parent))));
   ...
   function Get_CU_Access
     (A : in out CU_Metrix.Vector; S : CU_Symbol) return CU_Metrix
     .Element_Access;
   ...
   function File_Name_To_Print
     ...
     (if Arg (Cmd, Short_File_Names) then Directories.Simple_Name (File_Name)
   ...
   function Should_Print
     (Metric : Metrics_Enum; Metrics_To_Compute : Metrics_Set; M : Metrix;
      Depth  : Positive; XML : Boolean) return Boolean;
   ...
   function Should_Print_Any
     (First, Last : Metrics_Enum; Metrics_To_Compute : Metrics_Set; M : Metrix;
      Depth       : Positive; XML : Boolean) return Boolean is
   ...
   procedure Print_Range
     (Name        : String; Metrics_To_Compute : Metrics_Set;
      First, Last : Metrics_Enum; M : Metrix; Depth : Positive);
   ...
   function Skip_Print_Metrix
     (Cmd   : Command_Line; Metrics_To_Compute : Metrics_Set; M : Metrix;
      Depth : Positive; Doing_Coupling_Metrics : Boolean) return Boolean;
   ...
   procedure Print_Metrix
     (Cmd : Command_Line; File_Name : String; Metrics_To_Compute : Metrics_Set;
      M   : Metrix; Depth : Positive);
   --  Print the metrics for one node in text form

   procedure XML_Print_Metrix_Vals
     (Metrics_To_Compute : Metrics_Set; First : Metrics_Enum;
      Last               : Metrics_Enum; M : Metrix; Depth : Positive);
   ...
   procedure Print_File_Metrics
     (Cmd : Command_Line; XML_File : in out Text_IO.File_Type; File_M : Metrix;
      Metrics_To_Compute : Metrics_Set; Object_Dir : String);
   ...
   function Push_New_Metrix
     (Tool             : in out Metrics_Tool'Class; Node : Ada_Node;
      Source_File_Name :        String_Ref := null) return Metrix_Ref
   is
      K : constant Ada_Node_Kind_Type :=
        (if Node.Is_Null then Null_Kind else Kind (Node));

      procedure Set_CU_Metrix
        (M          : Metrix_Ref; Outer_Unit_Kind : Ada_Node_Kind_Type;
         Is_Subunit : Boolean);
      ...
      --  it's a spec or a body.
         ...
         case Outer_Unit_Kind is
            when Ada_Generic_Package_Decl | Ada_Package_Decl |
              Ada_Single_Protected_Decl | Ada_Protected_Type_Decl |
              Ada_Single_Task_Decl | Ada_Task_Type_Decl |
              Ada_Generic_Subp_Instantiation |
              Ada_Generic_Package_Instantiation | Ada_Generic_Renaming_Decl |
              Ada_Package_Renaming_Decl | Ada_Abstract_Subp_Decl |
              Ada_Null_Subp_Decl | Ada_Subp_Renaming_Decl =>
               pragma Assert (S = null);
               S         := M;
               M.Is_Spec := True;

            when Ada_Package_Body | Ada_Protected_Body | Ada_Entry_Body |
              Ada_Task_Body =>
   ...
   begin
      return Result : constant Metrix_Ref := new Metrix (Kind => K) do
         ...
         if not Node.Is_Null then
            ...
            begin
               ...
               if K = Ada_Compilation_Unit then
                  ...
                  Result.LI_Sub    :=
                    (if Length (Tool.Metrix_Stack) = 3 then
         ...
         if not Node.Is_Null and then Kind (Node) = Ada_Package_Body
               ...
               case Kind (Id) is
                  when Ada_Identifier =>
                     ...
                     begin
                        if Text = Pre then
                           return Other_Assertion;
                        elsif Text = Contract_Cases or else Text = Post then
   ...
   begin
      ...
      case Kind (Node) is
         ...
         when Ada_Abstract_Subp_Decl | Ada_Subp_Renaming_Decl |
           Ada_Subp_Decl =>
            declare
               R : constant Type_Expr := F_Subp_Returns (Get_Subp_Spec (Node));
            begin
   ...
   begin
      return Replace_String
          (Strip_Suffix (To_Lower (Kind'Img), Suffix => "_knd"), From => "_",
           To => " ");
   ...
   begin
      ...
      return
        ((if M.Is_Private_Lib_Unit then "private " else "") &
         Replace_String
           (Fine_Kind_String (M.Knd), From => "instantiation",
            To                             => "instance"));
   ...
   begin
      ...
      case Metric is
         when Lines | Lines_Code | Lines_Comment | Lines_Eol_Comment |
           Lines_Blank | Lines_Ratio =>
            return True;
         when Lines_Spark =>
            return Depth in 1 | 2;
         ...
         when Complexity_Statement | Complexity_Expression |
           Complexity_Cyclomatic | Complexity_Essential | Loop_Nesting =>
      ...
      then
         ...
         declare
            ...
            Num : constant Metric_Nat :=
              (if Metric = Lines_Average then
                 (if M.Vals (Num_Bodies) = 0 then 1 else M.Vals (Num_Bodies))
   ...
   begin
      ...
      if M.Kind = Ada_Compilation_Unit and then Doing_Coupling_Metrics
   ...
   begin
      ...
      if Doing_Coupling_Metrics then
         ...
         Put
           ("\n\n\1 (\2\3 at lines  \4)\n", Str (M.Text_Name).S,
            Fine_Kind_String (M.Knd), Str (M.LI_Sub).S, Lines_String (M.Sloc));
      end if;
      ...
      if Doing_Coupling_Metrics and then Depth = 3 then
         ...
         then
            ...
            for I in Coupling_Metrics loop
               if Should_Print (I, Metrics_To_Compute, M, Depth, XML => False)
               then
      ...
      Print_Range
        ("=== Code line metrics ===", Metrics_To_Compute, Lines_Metrics'First,
         Lines_Metrics'Last, M, Depth);

      Print_Range
        ("=== Contract metrics ===", Metrics_To_Compute,
         Contract_Metrics'First, Contract_Metrics'Last, M, Depth);

      Print_Range
        ("=== Element metrics ===", Metrics_To_Compute, Syntax_Metrics'First,
         Syntax_Metrics'Last, M, Depth);

      if M.Kind /= Ada_Compilation_Unit then
         Print_Range
           ("=== Complexity metrics ===", Metrics_To_Compute,
      ...
      if M.Kind = Ada_Compilation_Unit and then M.Num_With_Complexity > 0 then
         Print_Range
           (Average_Complexity_Metrics, Metrics_To_Compute,
            Complexity_Metrics'First, Complexity_Metrics'Last, M, Depth);
      end if;
   ...
   is
      ...
      To_Print_First : constant Metrics_Set :=
        ...
        (if M.Kind = Ada_Compilation_Unit then not Complexity_Only
      ...
      if M.Kind = Ada_Compilation_Unit then
         if Debug_Flag_V and Doing_Coupling_Metrics then
            for Sym of M.Depends_On loop
               Put
                 ("<dependence dep=""\1 \2 depends on \3"">\n",
                  Str (M.CU_Name).S, (if M.Is_Spec then "spec" else "body"),
                  Str (Sym).S);
            end loop;
            Put ("\n");
         end if;
   ...
   --  Can't pass Suffix as Extension, because that inserts an extra "."
      ...
      if Gen_Text (Cmd) then
         ...
         Print_Metrix (Cmd, File_Name, Metrics_To_Compute, File_M, Depth => 2);
   ...
   procedure Init (Tool : in out Metrics_Tool; Cmd : in out Command_Line) is
   ...
   procedure Print_Coupling
     (Cmd : Command_Line; Metrics_To_Compute : Metrics_Set);
   ...
   procedure Compute_Indirect_Dependencies (Global_M : Metrix) is
      ...
      procedure Visit (M : Metrix_Ref; Depends_On : in out CU_Symbol_Sets.Set);
   ...
   begin
      ...
      for Index in 1 .. Units_For_Coupling.Last_Index loop
         ...
         begin
            ...
            then
               Print_Metrix
                 (Cmd, File_M.Source_File_Name.all, Metrics_To_Compute, File_M,
                  Depth => 2);
   ...
   begin
      ...
      for File_M of Units_For_Coupling loop
         ...
         begin
            ...
            then
               XML_Print_Metrix
                 (Cmd, File_M.Source_File_Name.all, Metrics_To_Compute,
                  File_M.all, Depth => 2);
            end if;
         end;
      end loop;
      ...
      begin
         if Gen_Text (Cmd) and then Metrics_To_Compute (Metric) then
            if Global_M.Vals (Metric) > 0 then
               Put
                 (T, Val_To_Print (Metric, Global_M.all, XML => False),
   ...
   begin
      ...
      if Gen_Text (Cmd) then
         ...
         Print_Range
           ("Line metrics " & Summed, Metrics_To_Compute, Lines_Metrics'First,
            Lines_Metrics'Last, Global_M.all, Depth => 1);
         Print_Range
           ("Contract metrics " & Summed, Metrics_To_Compute,
            Contract_Metrics'First, Contract_Metrics'Last, Global_M.all,
            Depth => 1);
         Print_Range
           ("Element metrics " & Summed, Metrics_To_Compute,
            Syntax_Metrics'First, Syntax_Metrics'Last, Global_M.all,
            Depth => 1);

         Print_Computed_Metric
           ("\n\n \1 public types in \2 units", Public_Types,
            Computed_Public_Types);
         Print_Computed_Metric
           ("\n\n \1 type declarations in \2 units", All_Types,
            Computed_All_Types);
         Print_Computed_Metric
           ("\n\n \1 public subprograms in \2 units", Public_Subprograms,
            Computed_Public_Subprograms);
         Print_Computed_Metric
           ("\n\n \1 subprogram bodies in \2 units", All_Subprograms,
         ...
         then
            ...
            Put
              ("Average cyclomatic complexity: \1",
               Val_To_Print
                 (Complexity_Cyclomatic, Global_M.all, XML => True));
         end if;
   ...
   procedure Per_File_Action
     (Tool  : in out Metrics_Tool; Cmd : Command_Line; File_Name : String;
...
--    pragma Assert (Children_Count (CU_List) = 1);
      ...
      procedure Gather_Extra_Exit_Points (Node : Ada_Node; M : in out Metrix);
      ...
      package EC_Vectors is new Utils.Vectors (EC_Index, EC_Rec, EC_Array);
      use EC_Vectors;
         ...
         if Node = M.Node then
            declare
               ...
               Start : constant Slocs.Line_Number :=
                 (if Kind (Node) = Ada_Compilation_Unit then 1
                  else M.Sloc.Start_Line);

               Stop : constant Slocs.Line_Number :=
                 (if Kind (Node) = Ada_Compilation_Unit then Last (Cumulative)
               ...
               for Metric in Cumulative_Metrics loop
                  declare
                     Range_Count : Metric_Nat :=
                       Line_Range_Count
                         (Cumulative, First_Line => Start, Last_Line => Stop,
                          Metric                 => Metric);
                  begin
                     ...
                     if Metric = Lines_Code and then Should_Gather_Body_Lines
                     then
                        ...
                        then
                           Inc
                             (Global_M.Vals (Lines_Code_In_Bodies),
                              By => Range_Count);
                        end if;
      ...
      is
         ...
         function Find_Pragma (L : Ada_Node_List'Class) return Boolean;
         ...
         begin
            case Kind (P) is
               ...
               when Ada_Package_Decl | Ada_Single_Protected_Decl |
                 Ada_Protected_Type_Decl | Ada_Single_Task_Decl |
                 Ada_Task_Type_Decl | Ada_Generic_Package_Decl =>
         ...
         function Find_Pragma (L : Ada_Node_List'Class) return Boolean is
         begin
      ...
      begin
         ...
         Range_Count :=
           Line_Range_Count
             (Cumulative, First_Line => Sloc_Range (Section).Start_Line,
              Last_Line              => Sloc_Range (Section).End_Line,
              Metric                 => Lines_Code);
         if ON then
            case Kind (Section) is
               when Ada_Compilation_Unit | Ada_Package_Decl |
                 Ada_Single_Protected_Decl | Ada_Protected_Type_Decl |
                 Ada_Single_Task_Decl | Ada_Task_Type_Decl |
                 Ada_Generic_Package_Decl | Ada_Body_Node |
                 Ada_Abstract_Subp_Decl | Ada_Subp_Decl |
      ...
      procedure Gather_Contract_Metrics (Node : Ada_Node) is
         ...
         procedure Search_Aspects
           (Subp_Decl : Ada_Node; Has_Contracts, Has_Post : out Boolean);
      ...
      begin
         ...
         if Last_Index (Metrix_Stack) = 3 or else In_Visible_Part then
            if Node = M.Node and then not M.Is_Private_Lib_Unit then
               ...
               if Last_Index (Metrix_Stack) = 3
                 ...
                 and then M.Kind = Ada_Subp_Body and then Acts_As_Spec (M)
               then
                  Inc_All (Public_Subprograms);
               end if;
            end if;
         ...
         if Node.Kind in Ada_Stmt and then Node.Kind not in Ada_Label
      ...
      procedure Gather_Dependencies (Node : Ada_Node) is
         ...
         begin
            case Kind (Node) is
               ...
               when Ada_Generic_Subp_Instantiation | Ada_Subp_Renaming_Decl |
      ...
      begin
         ...
         if B and then False -- ???See P907-045
         then
            pragma Assert
              (Node.Parents (3) =
   ...
   begin
      ...
      if False then -- Disable this for now
         Put
           (" --incremental -- incremental processing on a per-file basis\n");
      end if;
      ...
      Put
        (" --no-local-metrics - do not compute detailed metrics for local program units\n");
      Put ("\n");

      Put
        ("Options to control metrics to compute. An option --<metric> turns computing\n");
      Put
        ("the metric ON, the corresponding --no-<metric> option turns computing the\n");
      Put
        ("metric OFF. If no metric option is specified, all the metrics are computed\n");
      Put
        ("and reported. If at least one positive option is  specified, only explicitly\n");
      ...
      Put
        ("  --contract-cyclomatic - McCabe Cyclomatic Complexity of contracts\n");
      Put ("\n");
      ...
      Put
        ("  --complexity-average    - average McCabe Cyclomatic Complexity of a body\n");
      Put ("  --loop-nesting          - maximal loop nesting level\n");
      Put
        ("  --no-static-loop        - do not count static loops for cyclomatic complexity\n");
      Put
        ("  --no-treat-exit-as-goto - do not consider exit statements as gotos when\n");
      ...
      Put
        ("  --lines-eol-comment - number of code lines also containing comments\n");
      ...
      Put
        ("  --lines-spark       - code lines with SPARK_Mode (not included in --lines-all)\n");
      Put ("\n");
      ...
      Put
        ("  --public-subprograms - number of public subprograms in a compilation unit\n");
      Put
        ("  --all-subprograms    - number of subprograms in a compilation unit\n");
      Put
        ("  --public-types       - number of public types in a compilation unit\n");
      ...
      Put
        ("  --hierarchy-coupling-out - hierarchy (category) fan-out coupling\n");
      Put
        ("  --hierarchy-coupling-in  - hierarchy (category) fan-in coupling\n");
      ...
      Put
        ("  --output-dir=dirname     - put files with detailed metrics into 'dirname'\n");
      Put ("  --generate-xml-output    - generate XML output\n");
      Put
        ("  --generate-xml-schema    - generate XML output and corresponding schema file\n");
      Put
        ("  --no-text-output         - do not generate output in text form\n");
      Put
        ("  --output-suffix=file-suffix - suffix for the detailed metrics file\n");
      Put
        ("  --global-file-name=filename - name of the global metrics file\n");
      Put ("  --xml-file-name=filename    - name of the XML output file\n");
      Put
        ("  --short-file-names          - use short source file names in output\n");
      Put ("\n");
      Put (" filename         - name of Ada source file for which metrics\n");
      Put ("                    should be computed\n");
      Put
        (" --files=filename - name of a text file containing a list of Ada\n");
      Put
        ("                    source files for which metrics should be computed\n");
      pragma Style_Checks ("M79");
   end Tool_Help;
   ...
   begin
      ...
      pragma Assert (Totals (Unit_Coupling_Out) = Totals (Unit_Coupling_In));
   ...
   begin
      ...
      for Sym of M.Depends_On loop
         Put
           ("<dependence dep=""\1 \2 depends on \3"">\n", Str (M.CU_Name).S,
            (if M.Is_Spec then "spec" else "body"), Str (Sym).S);
      end loop;
