...
package body Pp.Buffers is
   ...
   begin
      Append
        (Buf.Markers,
         Marker_Rec'
           (Position => Last_Index (Buf.To) + 1, Flag => Buf.To_Flag,
            Name     => Name));
...
--         end if;
   ...
   begin
      ...
      Put
        ("To[\1..\2] = <<\3>>end To[\1..\2]\n", "1",
         Image (Last_Index (Buf.To)), To_UTF8 (To_Array (Buf.To)));
      Put
        ("From[\1..\2] = <<\3>>end From[\1..\2]\n", Image (Buf.From_First),
         Image (Last_Index (Buf.From)), To_UTF8 (Valid_From (Buf)));
   ...
   begin
      ...
      Put
        ("\1 --> \2 '\3'  \4\n", Image (Rec.Position), Image (Pos),
         To_UTF8 ((1 => Rec.Name)), Image (Modular (Mark)));
   ...
   begin
      ...
      if Rec.Position = Buf.From_First then
         ...
         Rec :=
           (Flag => not Rec.Flag, Position => Last_Index (Buf.To) + 1,
            Name => Rec.Name);
...
--         end if;
   ...
   overriding procedure Initialize (Buf : in out Buffer) renames
     Initialize_Buffer;
...
--      Buf.Cur_Column := 1;
   ...
   procedure Insert_Keeping_Mark
     (Buf : in out Buffer; Mark : Marker; C : W_Char)
   is
   begin
...
--      end if;
   ...
   begin
      return
        Result : constant Boolean :=
          Is_Empty (Buf.From) and then Is_Empty (Buf.To) do
   ...
   begin
      ...
      Append
        (Buf.Markers,
         Marker_Rec'
           (Position => Last_Index (Buf.To), Flag => Buf.To_Flag,
            Name     => Name));
   ...
   is
      ...
      function At_Brackets_Start return Boolean is
        (Ptr + 2 <= Input'Last and then Input (Ptr + 1) = '"'
   ...
   begin
      ...
      Insert_Ada_Source
        (Buf, Input (First .. Input'Last), Wide_Character_Encoding,
         Expand_Tabs);
