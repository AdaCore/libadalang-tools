...
package body Stub.Actions is
   ...
   procedure Init (Tool : in out Stub_Tool; Cmd : in out Command_Line) is
   ...
   procedure Generate
     (Tool : in out Stub_Tool; Cmd : Command_Line; File_Name : String;
      Input :        String; BOM_Seen : Boolean; Root_Node : Ada_Node;
   ...
   Args : aliased GNAT.OS_Lib.Argument_List :=
     (Syntax_Only_Arg'Access, Comments_Fill_Arg'Access,
      Decimal_Grouping_Arg'Access, Based_Grouping_Arg'Access);

   function Get_Pp_Cmd return Command_Line is
   begin
      return Result : Command_Line (Pp.Command_Lines.Descriptor'Access) do
         Parse
           (Args'Access, Result, Cmd_Line_1, Null_Callback'Access,
   ...
   begin
      case N.Kind is
         when Ada_Package_Decl | Ada_Generic_Package_Decl =>
            ...
            begin
               return (not VP.Is_Null and then Recurse (F_Decls (VP)))
                 or else (not PP.Is_Null and then Recurse (F_Decls (PP)));
            end;
   ...
   is
         ...
         if Arg (Cmd, Comment_Header_Spec) then
            ...
            begin
               ...
               while Next <= Input'Last - 2 loop
                  while Next <= Input'Last - 2
                    and then Input (Next) in ' ' | ASCII.HT
                  loop
                     Next := Next + 1;
                  end loop;
               ...
               Append
                 (Out_Vec,
                  Replace_String
                    (Input (1 .. Last), From => "S p e c", To => "B o d y"));
               Put ("\n");
            end;
         end if;
      ...
      is
         ...
         Overrides             : constant Ada_Overriding_Node :=
           (if Decl.Kind in Ada_Classic_Subp_Decl then
              Decl.As_Classic_Subp_Decl.F_Overriding
            else Ada_Overriding_Unspecified);
         Returns : constant Boolean := not F_Subp_Returns (Spec).Is_Null;
      begin
         Pp.Actions.Format_Vector
           (Pp_Cmd, Input => Empty_Vec, Node => Ada_Node (Spec),
            In_Range      => (1, 0), Output => Pp_Out_Vec,
            Out_Range     => Tool.Ignored_Out_Range,
            Messages      => Tool.Ignored_Messages);
         pragma Assert (Is_Empty (Tool.Ignored_Messages));
         Put
           (" \1\2 is", Overriding_String (Overrides),
      ...
      procedure Generate_Entry_Body (Decl : Ada_Node; Name : W_Str) is
         Empty_Vec, Pp_Out_Vec : Char_Vector;
         Parms : constant Params := Decl.As_Entry_Decl.F_Spec.F_Entry_Params;
         Overrides             : constant Ada_Overriding_Node :=
           Decl.As_Entry_Decl.F_Overriding;
      begin
         if not Parms.Is_Null then
            Pp.Actions.Format_Vector
              (Pp_Cmd, Input => Empty_Vec, Node => Ada_Node (Parms),
               In_Range      => (1, 0), Output => Pp_Out_Vec,
               Out_Range     => Tool.Ignored_Out_Range,
               Messages      => Tool.Ignored_Messages);
            pragma Assert (Is_Empty (Tool.Ignored_Messages));
            Put
              ("\1entry \2 \3 when Standard.True is\n",
               Overriding_String (Overrides), Name,
               From_UTF8 (Elems (Pp_Out_Vec) (1 .. Last_Index (Pp_Out_Vec))));
         else
            Put
              ("\1entry \2 when Standard.True is\n",
               Overriding_String (Overrides), Name);
         end if;
      ...
      begin
         ...
         if Arg (Cmd, No_Exception) and then not Returns then
            ...
            Put
              ("\1raise Program_Error with ""Unimplemented \2 \3"";\n",
            ...
            for X in 1 .. Last_Child_Index (Decls) loop
               ...
               begin
                  ...
                  then
                     ...
                     begin
                        if Next_Part.Is_Null or else Next_Part = Subtree then
      ...
      begin
         ...
         case Decl.Kind is
            ...
            when Ada_Subp_Decl | Ada_Generic_Subp_Decl | Ada_Single_Task_Decl |
              Ada_Task_Type_Decl | Ada_Entry_Decl | Ada_Task_Body_Stub |
              Ada_Subp_Body_Stub =>
               null;
         ...
         for Child of Local_Decls loop
            ...
            then
               ...
               Generate
                 (Tool, Cmd, File_Name, Input, BOM_Seen, Root_Node => Child,
         ...
         case Decl.Kind is
            when Ada_Package_Decl | Ada_Generic_Package_Decl |
              Ada_Package_Body_Stub | Ada_Single_Protected_Decl |
              Ada_Protected_Type_Decl | Ada_Protected_Body_Stub =>
               Put ("\nend;\n");
            when Ada_Subp_Decl | Ada_Generic_Subp_Decl | Ada_Subp_Body_Stub |
              Ada_Single_Task_Decl | Ada_Task_Type_Decl | Ada_Task_Body_Stub |
              Ada_Entry_Decl | Ada_Subp_Body | Ada_Package_Body |
      ...
      begin
         ...
         Pp.Actions.Format_Vector
           (Pp_Cmd, Input => Out_Vec, Node => Root (Out_Unit),
            In_Range      => (1, 0), Output => Pp_Out_Vec,
            Out_Range     => Tool.Ignored_Out_Range,
            Messages      => Tool.Ignored_Messages);
         pragma Assert (Is_Empty (Tool.Ignored_Messages));
      ...
      function Get_Output_Name (Resolve_Links : Boolean) return String is
         ...
         begin
            ...
            then
               ...
               Cmd_Error
                 ("output file name should be provided because " & File_Name &
                  " does not follow GNAT naming rules for " & "spec files");
            end if;
         end Default_Name;
            ...
            return
              Result : constant String :=
                +GNATCOLL.Projects.File_From_Unit
                  (Project => Project (Arg_File_Info), Unit_Name => Unit_Name,
                   Part => Part, Language => "Ada", File_Must_Exist => False)
            do
               null;
            end return;
         ...
         return
           Result : constant String :=
             (if Arg (Cmd, Output_Directory) = null then
                Compose (Containing_Directory (File_Name), Simple)
      ...
      begin
         if Out_File = Invalid_FD then
            raise Program_Error with "write of " & Output_Name & " failed";
         end if;
      ...
      begin
         if Root_Node.Kind not in Ada_Package_Decl | Ada_Generic_Package_Decl
         then
         ...
         begin
            ...
            Append
              (Pp_Out_Vec, Old_Content (Insertion_Index .. Old_Content'Last));
            Free (Old_Content);
            Clear (Out_Vec);
         end;
...
--         PP_Trivia (Unit);
      ...
      case Root_Node.Kind is
         when Ada_Package_Decl | Ada_Generic_Package_Decl | Ada_Subp_Decl |
           Ada_Generic_Subp_Decl =>
            if Arg (Cmd, Subunits) and then not Update_Body_Specified (Cmd)
            then
               ...
               begin
                  ...
                  Cmd_Error_No_Help
                    ("output file name should be provided because " &
                     File_Name & " does not follow GNAT naming rules for " &
                     "spec files");
               end;
            end if;
         when others =>
   ...
   begin
      ...
      Put
        ("  --subunits              generate separate bodies for body stubs\n");
      Put
        ("  --files=filename        name of a file containing a list of files to process\n");
      Put ("\n");

      Put
        ("  --force                replace an existing body file (if any), with a body sample\n");
      Put ("  --max-line-length=nnn  maximum line length in sample body\n");
      Put
        ("  --indentation=n        number of spaces used for indentation in sample body\n");
      Put ("  --alphabetical-order   alphabetically order local bodies\n");
      Put ("  --comment-header-sample insert a sample comment header\n");
      Put
        ("  --comment-header-spec  insert the comment header from the spec\n");
      Put
        ("  --header-file=filename insert the comment header from the specified file\n");
      Put
        ("  --no-exception         avoid raising Program_Error in procedure stubs\n");
      Put
        ("  --no-local-header      no local comment headers for unit stubs\n");
      Put
        ("  --output=body-name     the name of the file to place the body into\n");
      Put
        ("  --output-dir=directory place generated file(s) into directory\n");
      Put ("  --wide-character-encoding=(8|b)\n");
      Put
        ("                         wide character encoding of the result file\n");
