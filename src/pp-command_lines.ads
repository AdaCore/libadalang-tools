...
package Pp.Command_Lines is
   ...
   package Pp_Disable is new
     Disable_Switches (Descriptor, [1 => To_All (Rep_Clauses)]);
   ...
   package Pp_Flag_Shorthands is new
     Pp_Flag_Switches.Set_Shorthands
       ([Obsolete_A0           => +"-A0",
         Obsolete_A1           => +"-A1",
         Obsolete_A2           => +"-A2",
         Obsolete_A3           => +"-A3",
   ...
   package Pp_Flag_Shorthands_N is new
     Pp_Flag_Switches.Set_Shorthands
       ([No_Tab => +"-N",
         --  Not documented
         others => null]);
   ...
   package Attribute_Casing_Switches is new
     Enum_Switches (Descriptor, Attribute_Casing);

   package Attribute_Casing_Shorthands is new
     Attribute_Casing_Switches.Set_Shorthands
   ...
   package Keyword_Casing_Switches is new
     Enum_Switches (Descriptor, Keyword_Casing);

   package Keyword_Casing_Shorthands is new
     Keyword_Casing_Switches.Set_Shorthands
   ...
   package Name_Casing_Shorthands is new
     Name_Casing_Switches.Set_Shorthands
   ...
   package Enum_Casing_Shorthands is new
     Enum_Casing_Switches.Set_Shorthands
   ...
   package Type_Casing_Shorthands is new
     Type_Casing_Switches.Set_Shorthands
   ...
   package Number_Casing_Switches is new
     Enum_Switches (Descriptor, Number_Casing);

   package Number_Casing_Shorthands is new
     Number_Casing_Switches.Set_Shorthands
   ...
   package Pragma_Casing_Switches is new
     Enum_Switches (Descriptor, Pragma_Casing);

   package Pragma_Casing_Shorthands is new
     Pragma_Casing_Switches.Set_Shorthands
   ...
   package Constant_Casing_Switches is new
     Enum_Switches (Descriptor, Constant_Casing);

   package Constant_Casing_Shorthands is new
     Constant_Casing_Switches.Set_Shorthands
   ...
   package Pp_Boolean_Switches is new
     Boolean_Switches (Descriptor, Pp_Booleans);
   ...
   package Pp_Boolean_Shorthands is new
     Pp_Boolean_Switches.Set_Shorthands
       ([Alignment                     => null,
         ...
         Comments_Standard_Indentation => +"-c2",
         --  not documented
   ...
   package Pp_String_Syntax is new
     Pp_String_Switches.Set_Syntax
       ([File_Name_File => '=',
         Output         => '=',
         Output_Force   => '=',
         End_Of_Line    => '=',
   ...
   package Pp_String_Shorthands is new
     Pp_String_Switches.Set_Shorthands
       ([File_Name_File => null,
         Output         => +"-o",
         Output_Force   => +"-of",
         End_Of_Line    => +"--eol",
   ...
   package Pp_String_Seq_Switches is new
     String_Seq_Switches (Descriptor, Pp_String_Seqs);

   package Pp_String_Seq_Syntax is new
     Pp_String_Seq_Switches.Set_Syntax ([Dictionary => ':']);

   package Pp_String_Seq_Shorthands is new
     Pp_String_Seq_Switches.Set_Shorthands ([Dictionary => +"-D"]);
   ...
   package Pp_Nat_Switches is new
     Other_Switches
       (Descriptor,
        Pp_Nats,
        Natural,
   ...
   package Pp_Nat_Syntax is new
     Pp_Nat_Switches.Set_Syntax
       ([Max_Line_Length     => '!',
         Indentation         => '!',
         Indent_Continuation => '!',
         Initial_Indentation => '=',
   ...
   package Pp_Nat_Defaults is new
     Pp_Nat_Switches.Set_Defaults
       ([Max_Line_Length     => 79,
         Indentation         => 3,
   ...
   package Pp_Nat_Shorthands is new
     Pp_Nat_Switches.Set_Shorthands
       ([Max_Line_Length     => +"-M",
         Indentation         => +"-i",
   ...
   type Layout_Kind is (Default, Minimal, Tall, Compact);

   package Layout_Switches is new
     Other_Switches
       (Descriptor,
        PP_Layout,
        Layout_Kind,
   ...
   package Layout_Syntax is new Layout_Switches.Set_Syntax ([Layout => '=']);

   package Layout_Defaults is new
     Layout_Switches.Set_Defaults ([Layout => Default]);

   package Layout_Shorthand is new
     Layout_Switches.Set_Shorthands ([Layout => +"-L"]);
   ...
   function Obsolete_Alignment_Switch (Cmd : Command_Line) return Boolean
   is (Arg (Cmd, Obsolete_A1)
   ...
   function Comment_Filling_Enabled (Cmd : Command_Line) return Boolean
   is (not Arg (Cmd, Comments_Unchanged) and Arg (Cmd, Comments_Fill));

   function Insert_Blank_Lines (Cmd : Command_Line) return Boolean
   is (Arg (Cmd, Insert_Blank_Lines) and not Arg (Cmd, Preserve_Blank_Lines));

   function Preserve_Blank_Lines (Cmd : Command_Line) return Boolean
   is (not Arg (Cmd, Insert_Blank_Lines) and Arg (Cmd, Preserve_Blank_Lines));
   ...
   subtype Lower_Upper_PP_Casing is PP_Casing
   with Predicate => Lower_Upper_PP_Casing in Lower_Case | Upper_Case;

   subtype Lower_Upper_Mixed_PP_Casing is PP_Casing
   with
     Predicate =>
       Lower_Upper_Mixed_PP_Casing in Lower_Case | Upper_Case | Mixed;

   subtype Cmd_Line is Command_Line;

   function PP_Attribute_Casing
     (Cmd : Cmd_Line) return Lower_Upper_Mixed_PP_Casing
   is (case Attribute_Casing'(Arg (Cmd)) is
         when Attribute_Mixed_Case => Mixed,
         when Attribute_Lower_Case => Lower_Case,
         when Attribute_Upper_Case => Upper_Case);

   function PP_Keyword_Casing (Cmd : Cmd_Line) return Lower_Upper_PP_Casing
   is (case Keyword_Casing'(Arg (Cmd)) is
         when Keyword_Lower_Case => Lower_Case,
         when Keyword_Upper_Case => Upper_Case);

   function PP_Pragma_Casing
     (Cmd : Cmd_Line) return Lower_Upper_Mixed_PP_Casing
   is (case Pragma_Casing'(Arg (Cmd)) is
   ...
   function PP_Number_Casing (Cmd : Cmd_Line) return PP_Casing
   ...
   function PP_Indentation (Cmd : Cmd_Line) return Positive
   is (Arg (Cmd, Indentation));
