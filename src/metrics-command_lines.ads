...
package METRICS.Command_Lines is
   ...
   use Common_Flag_Switches, Common_String_Seq_Switches;

   package Metrics_Disable is new
     Disable_Switches
       (Common_Descriptor,
   ...
   package Metrics_Common_String_Shorthands is new
   ...
   package Metrics_Flag_Switches is new
     Flag_Switches (Descriptor, Metrics_Flags);

   package Metrics_Flag_Shorthands is new
     Metrics_Flag_Switches.Set_Shorthands
       ([Test                    => null,
   ...
   subtype Metrics_Enum is
     Metrics_Booleans
       range Metrics_Booleans'First .. Metrics_Booleans'Pred (Metrics_All);
   ...
   function Empty_Metrics_Set return Metrics_Set
   is ([others => False]);
   function All_Metrics_Set return Metrics_Set
   is ([others => True]);
   ...
   package Metrics_Boolean_Switches is new
     Boolean_Switches (Descriptor, Metrics_Booleans);
   ...
   package Metrics_Boolean_Shorthands is new
     Metrics_Boolean_Switches.Set_Shorthands
       ...
       --  shorthands do not have exactly the same semantics as the long forms.
       --  For example, -lratio seems to turn on --complexity-average, whereas
       --  --lines-ratio does not. We don't need the complexity of those
       --  variations.
       --
       --  The old version also had -nocc (turns off Complexity_Cyclomatic),
       --  -noec (turns off Complexity_Essential), and -nonl (turns off
       --  Loop_Nesting). We no longer support those.
       ([Lines                  => +"-la",
         Lines_Code             => +"-lcode",
         Lines_Comment          => +"-lcomm",
   ...
   package Metrics_String_Switches is new
     String_Switches (Descriptor, Metrics_Strings);

   package Metrics_String_Syntax is new
     Metrics_String_Switches.Set_Syntax
       ([Output_Suffix => '=', Global_File_Name => '=', Xml_File_Name => '=']);

   package Metrics_String_Shorthands is new
     Metrics_String_Switches.Set_Shorthands
       ([Output_Suffix    => +"-o",
         Global_File_Name => +"-og",
         Xml_File_Name    => +"-ox"]);
   ...
   package Metrics_String_Seq_Switches is new
     String_Seq_Switches (Descriptor, Metrics_String_Seqs);

   package Metrics_String_Seq_Syntax is new
     Metrics_String_Seq_Switches.Set_Syntax ([Gnatmetric_Debug => '!']);

   package Metrics_String_Seq_Shorthands is new
     Metrics_String_Seq_Switches.Set_Shorthands
       ([Gnatmetric_Debug => +"-debug"]);

   package Freeze is new Freeze_Descriptor (Descriptor);

   use Metrics_Flag_Switches, Metrics_String_Switches;

   ----------------

   function Gen_XML (Cmd : Command_Line) return Boolean
   is (Arg (Cmd, Generate_XML_Output)
       or else Arg (Cmd, Generate_XML_Schema)
       or else Arg (Cmd, No_Text_Output)
       or else Present (Arg (Cmd, Xml_File_Name)));

   function Gen_Text (Cmd : Command_Line) return Boolean
   is (not Arg (Cmd, No_Text_Output));
