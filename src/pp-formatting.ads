...
package Pp.Formatting is
   ...
   type Line_Break is record
      ...
      Removed : Boolean; -- True if removed by Remove_Duplicates
...
--  ????      Kind     : Ada_Tree_Kind;
   ...
   type Tab_Rec is record
      ...
      Mark : Marker;
      --  Position in the buffer of the tab???get rid of
   ...
   type Lines_Data_Rec is record
      ...
      --  New_Tokns is the new tokens generated by certain phases. It should
      --  match Out_Tokns. To build a new value of New_Tokns plus some other
      --  tokens, we move New_Tokns to Saved_New_Tokns, loop through
      --  Saved_New_Tokns, appending those and newer ones onto New_Tokns. This
      --  ensures that index values pointing into New_Tokns will be correct
      --  after the pass.
   ...
   procedure Put_LBs (Out_Buf : Buffer; LB_Vec : Line_Break_Vector);
   ...
   --  above. (***) marks passes that directly modify the output (Out_Buf,
   --  New_Tokns).
   --
   --  Convert_Tree_To_Ada (***)
