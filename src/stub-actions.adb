...
package body Stub.Actions is
   ...
   function Has_Elaborate_Body (N : Ada_Node) return Boolean
   is (Exists (P_Get_Aspect (N.As_Basic_Decl, Elaborate_Body)));
   ...
   function Q (S : W_Str) return W_Str
   is (if S (S'First) = '"' then """" & S & """" else S);
   ...
   is
      ...
      Parent_Name           : constant W_Str :=
        Get_Parent_Name (Parent_Body_Of_Subunit);
      Root_Node_Name        : constant W_Str :=
        (if not Parent_Body_Of_Subunit.Is_Null then Parent_Name & "." else "")
      ...
      package Buffered_Output is new
      ...
      procedure Walk (Decl : Ada_Node; Level : Natural) is
         ...
         Name                          : constant W_Str :=
           Full_Name (Get_Def_Name (Decl).As_Name);
